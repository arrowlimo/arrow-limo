...
class KnowledgeRetriever:
    ...
    def __init__(
        self, db_path: str = None, collection_names: list[str] = None
    ) -> None:
        """
        ...
        self.db_path = db_path or os.path.join(
            os.path.dirname(__file__), "ai_knowledge_db"
        )
        ...
        try:
            ...
            for coll_name in self.collection_names:
                try:
                    self.collections[coll_name] = self.client.get_collection(
                        coll_name
                    )
                except Exception as e:
                    logger.warning(
                        "Collection %s not found: %s", coll_name, e
                    )
    ...
    def search(
        self, query: str, domain_filter: str = None, top_k: int = 5
    ) -> dict[str, Any]:
        """
        ...
        Args:
            ...
            domain_filter: Optional domain filter (tax, payroll, charter,
            banking, etc.)
        ...
        results = {"query": query, "total_results": 0, "collections": {}}
    ...
    def get_schema_info(self, table_name: str) -> dict[str, Any]:
        ...
        query = (
            f"Schema and columns for {table_name} table "
            f"with all fields and relationships"
        )
    ...
    def get_business_rule(self, rule_name: str) -> dict[str, Any]:
        ...
        Args:
            rule_name: Rule name (e.g., 'reserve_number',
                'GST calculation', 'WCB')
        ...
        query = (
            f"Business rule for {rule_name}: "
            f"definition, examples, and implementation"
        )
    ...
    def get_code_example(self, topic: str) -> dict[str, Any]:
        ...
        Args:
            topic: Code topic (e.g., 'GST calculation',
                'charter query', 'payroll')
        ...
        query = (
            f"Python or SQL code example for {topic} "
    ...
    def format_context(self, query: str, top_k: int = 3) -> str:
        ...
        for i, result in enumerate(results[:top_k], 1):
            context += f"{i}. [{result['source']}] (similarity: {result['similarity']:.1%})\n"
...
def test_retriever() -> None:
    ...
    print("Business Rule: reserve_number")
    print(f"Total Results: {results['total_results']}")
    ...
    print("Code Topic: payroll calculation")
    print(f"Total Results: {results['total_results']}")
    ...
    print(f"Query: {query}")
    print("Top Results (sorted by similarity):")
    for item in results[:3]:
        print(
            f"  - {item['similarity']:.2%}: [{item['source']}]"
            f"{item['text'][:70]}..."
        )
