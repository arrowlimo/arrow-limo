...
class FunctionExecutor:
    ...
    def __init__(
        self, user_role: str = "analyst", log_file: str | None = None
    ) -> None:
        """
        Initialize executor
    ...
    def parse_function_call(self, llm_response: str) -> list[dict[str, Any]]:
        ...
        for line in lines:
            ...
            try:
                ...
                args_str = call_str[
                    call_str.index("(") + 1 : call_str.rindex(")")
                ].strip()
                ...
                calls.append({"function": func_name, "args": args})

            except Exception as e:
                self.logger.warning(
                    "Failed to parse function call: %s - %s", call_str, e
                )

        return calls

    def execute_function_call(
        self, call: dict[str, Any], user_id: str | None = None
    ) -> ExecutionResult:
        """
    ...
    def answer_query_best_effort(self, query: str) -> dict[str, Any]:
        """Return a deterministic answer when the LLM is unavailable."""
        normalized = (query or "").strip()
        lowered = normalized.lower()

        routed = self._route_query_to_function(lowered)
        if routed:
            result = self.execute(routed["function"], routed["args"])
            if result.success:
                return {
                    "text": self._format_execution_result(normalized, result),
                    "functions": [
                        {
            ...
            return {
                "text": f"Fallback mode could not complete "
                    f"`{routed['function']}`: {result.error}",
                "functions": [],
            }

        return {
            "text": self._build_policy_response(normalized),
            "functions": [],
        }

    def _route_query_to_function(
        self, lowered: str
    ) -> dict[str, Any] | None:
        year = self._extract_year(lowered) or datetime.now().year
        month = self._extract_month(lowered)

        if "trial balance" in lowered:
            return {
                "function": "get_trial_balance",
                "args": {"year": year, "month": month or datetime.now().month},
            }

        if "monthly summary" in lowered or (
            "summary" in lowered and month is not None
        ):
            return {
        ...
        if "unpaid charter" in lowered or "receivable" in lowered:
            return {
                "function": "get_unpaid_charters",
                "args": {
                    "start_date": f"{year}-01-01",
                    "end_date": f"{year}-12-31",
                },
        ...
        if (
            "missing deduction" in lowered
            or "uncategorized receipt" in lowered
        ):
            return {
                "function": "check_missing_deductions",
                "args": {"year": year},
            }
    ...
    def _extract_month(self, text: str) -> int | None:
        months = {
            name.lower(): idx for idx, name in enumerate(month_name) if idx
        }
    ...
    def _format_execution_result(
        self, query: str, result: ExecutionResult
    ) -> str:
        payload = result.result or {}
        if result.function_name == "get_trial_balance":
            return (
                f"Trial balance for {query}: debit"
                f"${payload.get('debit_total', 0):,.2f}, "
                f"credit ${payload.get('credit_total', 0):,.2f},"
                f"balanced={payload.get('balanced', False)}."
            )

        if result.function_name == "get_monthly_summary":
            notes = payload.get("notes") or []
            note_text = " ".join(notes[:2])
            return (
                f"{payload.get('period', 'Selected period')}: revenue"
                f"${payload.get('total_revenue', 0):,.2f}, "
                f"expenses ${payload.get('total_expenses', 0):,.2f}, net"
                f"${payload.get('net_revenue', 0):,.2f}. "
                f"Basis: {payload.get('revenue_basis', 'unknown')}."
                f"{note_text}"
            )

        if result.function_name == "calculate_wcb_owed":
            return (
                f"WCB estimate for "
                f"{payload.get('period', 'selected period')}: "
                f"gross payroll "
                f"${payload.get('gross_payroll', 0):,.2f}, owed"
                f"${payload.get('wcb_owed', 0):,.2f}, "
                f"paid ${payload.get('wcb_paid', 0):,.2f}, balance"
                f"${payload.get('wcb_balance', 0):,.2f}."
            )

        if result.function_name == "get_unpaid_charters":
            return (
                f"Unpaid charters for "
                f"{payload.get('period', 'selected period')}: "
                f"{payload.get('unpaid_count', 0)} open charter(s), total"
                f"outstanding "
                f"${payload.get('total_outstanding', 0):,.2f}."
            )

        if result.function_name == "check_missing_deductions":
            return (
                f"Missing deduction review for {payload.get('year', '')}: "
                f"{payload.get('uncategorized_count', 0)} uncategorized"
                f"receipt(s), "
                f"${payload.get('uncategorized_amount', 0):,.2f} affected."
            )

        return json.dumps(payload, default=str, indent=2)

    def _build_policy_response(self, query: str) -> str:
        lowered = query.lower()
        ...
        if any(
            ...
            matched_rules.append(
                "Revenue must come from charter_payments cash received, net"
                "of refunds and reversals. Do not use"
                "charters.total_amount_due or banking credits as T2 revenue."
            )

        if any(token in lowered for token in ["ei", "t4", "payroll"]):
            matched_rules.append(
                "Paul Richard and Michael Richard are EI-exempt. Force T4 Box"
                "18 and Box 24 to zero and set the EI exemption code."
            )

        if any(token in lowered for token in ["defer", "deferred", "paul"]):
            matched_rules.append(
                "Paul Richard is deferring all pay from 2012 onward. Treat"
                "those wages as deferred/unpaid unless a specific payment"
                "record shows they were actually paid."
            )

        if any(token in lowered for token in ["xml", "cra", "t4"]):
            matched_rules.append(
                "CRA T4 XML filing uses the working 2026V4 structure:"
                "required contact email in T619, omit empty optional fields,"
                "no BOM, and keep sbmt_ref_id to 8 alphanumeric characters."
            )
        ...
        return "Fallback mode: " + " ".join(matched_rules)
...
def test_executor() -> None:
    ...
    print("\n[TEST 3] Execute Read-Only Function")
    print("-" * 80)
    result = executor.execute("get_trial_balance", {"year": 2024, "month": 12})
    ...
    result = restricted_executor.execute(
        "update_lms_data", {"reserve_number": "123"}
    )
    print(f"Success: {result.success}")
    print(f"Error: {result.error}")
    ...
    CALL_FUNCTION: calculate_wcb_owed(start_date=2024-01-01,
    end_date=2024-12-31)
