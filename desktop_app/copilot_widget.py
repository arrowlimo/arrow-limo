...
class CopilotWorker(QThread):
    ...
    def __init__(self, query: str, rag_engine, llm_engine, executor) -> None:
        super().__init__()
        self.query = query
        self.rag = rag_engine
        self.llm = llm_engine
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if self.rag:
                _context_data = self.rag.search(self.query, top_k=3)
            ...
            if self.llm:
                ...
                if llm_response.error:
                    if self.executor:
                        fallback = self.executor.answer_query_best_effort(
                            self.query
                        )
                        self.response_ready.emit(
                            {
                                "text": fallback.get("text", ""),
                                "functions": fallback.get("functions", []),
                ...
                fallback = self.executor.answer_query_best_effort(self.query)
                response_text = fallback.get("text", "")
                functions_called = []
                function_results = fallback.get("functions", [])
            else:
            ...
            self.error.emit(f"Processing error: {e!s}")
...
class CopilotWidget(QWidget):
    ...
    def __init__(
        self, parent=None, rag_engine=None, llm_engine=None, executor=None
    ) -> None:
        """
    ...
    def _setup_quick_actions(self) -> None:
        ...
        actions = [
            (
                "Trial Balance (Dec)",
                "What is our trial balance for December 2024?",
            ),
            ...
            (
                "T4 Payroll Summary",
                "Summarize our T4 payroll and tax compliance",
            ),
    ...
    @pyqtSlot(str)
    def _on_progress(self, status: str) -> None:
        """Update progress"""
    ...
    def _add_chat_message(self, role: str, text: str, is_user: bool = False) -> None:
...
def test_widget() -> None:
    ...
    # Create main window
    window = __import__(
        "PyQt6.QtWidgets", fromlist=["QMainWindow"]
    ).QMainWindow()
