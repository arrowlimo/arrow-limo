...
Double-click to access full employee details, documents, pay, floats, expenses,
lunch tracking
"""
...
class EnhancedEmployeeListWidget(QWidget):
    ...
    def __init__(self, db) -> None:
        ...
        self.position_filter.addItems(
            ["All", "Driver", "Dispatcher", "Manager", "Admin"]
        )
    ...
    def load_data(self) -> None:
        ...
        try:
            with DatabaseContext(self.db, auto_commit=False) as cur:
                ...
                # Filter out garbage records: LEGACY metadata, pure
                # placeholders, etc.
                # Keep only those with real names (not text fragments like PDF
                # captions)
                cur.execute("""
                    SELECT
                        e.employee_id,
                    ...
                    dp.employee_id
                        AND EXTRACT(YEAR FROM dp.pay_date) = EXTRACT(YEAR FROM
                        CURRENT_DATE)
                    WHERE
                        ...
                        -- Placeholder drivers
                        AND e.full_name NOT ILIKE 'Driver D%'
                        -- Not empty
                        AND TRIM(COALESCE(e.full_name, '')) != ''
                        -- Not single letters
                        AND LENGTH(TRIM(COALESCE(e.full_name, ''))) > 2
            ...
            for i, row in enumerate(rows):
                ...
                self.table.setItem(
                    i, 3, QTableWidgetItem(str(hire_date or ""))
                )
                self.table.setItem(i, 4, QTableWidgetItem(str(status or "")))
                self.table.setItem(
                    i, 5, QTableWidgetItem("✓" if is_chauffeur else "")
                )
                self.table.setItem(
                    i, 6, QTableWidgetItem(f"${float(ytd_pay or 0):,.2f}")
                )
                self.table.setItem(
                    i, 7, QTableWidgetItem(f"${float(floats or 0):,.2f}")
                )
                self.table.setItem(
                    i, 8, QTableWidgetItem(str(int(missing or 0)))
                )

                # Highlight rows with issues
                if float(floats or 0) > 0:
                    self.table.item(i, 7).setBackground(
                        QColor(255, 200, 200)
                    )  # Red for unreturned floats
                if int(missing or 0) > 0:
                    self.table.item(i, 8).setBackground(
                        QColor(255, 200, 200)
                    )  # Red for missing receipts

            self.status_label.setText(f"Loaded {len(rows)} employees")
        except Exception as e:
            logger.error(f"Failed to load employees: {e}")
            QMessageBox.critical(
                self, "Error", f"Failed to load employees: {e}"
            )
    ...
    def apply_filters(self) -> None:
        ...
        for i in range(self.table.rowCount()):
            ...
            if (
                position_filter != "All"
                and position_filter.lower() not in position.lower()
            ):
                show = False
        ...
        self.status_label.setText(
            f"Showing {visible_count} of {self.table.rowCount()} employees"
        )

    def on_employee_double_clicked(self, index) -> None:
    ...
    def open_employee_detail(self, employee_id) -> None:
    ...
    def edit_selected(self) -> None:
        ...
        if row >= 0:
            ...
            QMessageBox.warning(
                self, "Warning", "Please select an employee first"
            )
            ...
            if reply == QMessageBox.StandardButton.Yes:
                try:
                    with DatabaseContext(self.db, auto_commit=True) as cur:
                        cur.execute(
                            "UPDATE employees SET employment_status ="
                            "'Terminated' WHERE employee_id = %s",
                            (emp_id,),
                        )
                    QMessageBox.information(
                        self, "Success", f"Employee {emp_name} terminated"
                    )
                    self.load_data()
                except Exception as e:
                    logger.error(f"Failed to terminate employee: {e}")
                    QMessageBox.critical(
                        self, "Error", f"Failed to terminate employee: {e}"
                    )
        else:
            QMessageBox.warning(
                self, "Warning", "Please select an employee first"
