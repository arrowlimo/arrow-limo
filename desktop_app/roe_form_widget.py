...
- One-click autofill from recent payroll history (employee_pay_master,
pay_periods)
- Printable ROE summary via QTextDocument (PDF)

Assumptions:
- employee data available in `employees` (full_name, employee_number,
SIN optional, hire_date, termination_date)
- payroll history in `employee_pay_master` with `gross_pay`,
`total_hours_worked`, `pay_period_id`
- pay period dates in `pay_periods` (period_start_date, period_end_date,
pay_date)
"""
...
class ROEFormWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        layout.addWidget(
            QLabel(
                "Select employee, then use One-Click Fill to auto-populate"
                "fields from recent payroll history."
            )
        )
    ...
    def _build_top_controls(self) -> object:
        row = QHBoxLayout()
    ...
    def _build_roe_fields_group(self) -> object:
        ...
        self.pay_period_type_combo.addItems(
        ...
        form.addRow(
            "Insurable Hours (last 52 weeks)", self.insurable_hours_spin
        )
        form.addRow(
            "Insurable Earnings (last 52 weeks)", self.insurable_earnings_spin
        )
    ...
    def autofill_roe(self) -> None:
        ...
        try:
            ...
            with DatabaseContext(self.db, auto_commit=False) as cur:
                cur.execute(
                    """
                    SELECT ppm.pay_period_id, pp.period_start_date,
                    pp.period_end_date, pp.pay_date,
                           COALESCE(ppm.total_hours_worked, 0),
                           COALESCE(ppm.gross_pay, 0)
                ...
                if last_row:
                    ...
                    self.last_pay_period_start.setDate(
                        QDate.fromString(str(start), "yyyy-MM-dd")
                    )
                    self.last_pay_period_end.setDate(
                        QDate.fromString(str(end), "yyyy-MM-dd")
                    )
                else:
                    pay_date = date.today()
                ...
                cur.execute(
                    """
                    SELECT COALESCE(SUM(ppm.total_hours_worked),
                    0) AS hours_sum,
            ...
            logger.error(f"Autofill error: {exc}")
            self._set_status(f"Autofill error: {exc}", error=True)

    # Print ------------------------------------------------------------------
    def preview_roe(self) -> None:
        try:
            ...
            QMessageBox.critical(
                self, "Preview Error", f"Failed to preview ROE:\n{exc}"
            )

    def print_roe(self) -> None:
        try:
            doc = self._build_document()
            ...
            QMessageBox.information(
                self, "Printed", f"ROE saved to:\n{filename}"
            )
        except Exception as exc:
            logger.error(f"Print error: {exc}")
            QMessageBox.critical(
                self, "Print Error", f"Failed to generate ROE PDF:\n{exc}"
            )

    def export_roe_csv(self) -> None:
        ...
        try:
            ...
            rows = [
                ...
                ["SIN", self.employee_sin_edit.text()],
                [""],
                ["Employment Period"],
                ["Last Pay Period Start", self.last_pay_period_start.date().toString("yyyy-MM-dd")],
                ["Last Pay Period End", self.last_pay_period_end.date().toString("yyyy-MM-dd")],
                ["Last Day Worked", self.last_day_edit.date().toString("yyyy-MM-dd")],
                [""],
                ["Income Information"],
                [
                    "Insurable Earnings (52w)",
                    f"${self.insurable_earnings_spin.value():,.2f}",
                ],
                ["Insurable Hours (52w)", f"{self.insurable_hours_spin.value():,.2f}"],
                [""],
                ["Reason for Separation"],
                ["Reason", self.reason_combo.currentText()],
                ["Pay Period Type", self.pay_period_type_combo.currentText()],
                [""],
            ...
            QMessageBox.information(
                self, "Success", f"✅ ROE exported to CSV:\n{filename}"
            )
        except Exception as e:
            QMessageBox.critical(self, "Error", f"CSV export failed: {e}")

    def export_roe_word(self) -> None:
        ...
        try:
            ...
            table.cell(2, 1).text = self.employee_sin_edit.text()
            table.cell(3, 0).text = "Last Pay Period Start"
            table.cell(3, 1).text = self.last_pay_period_start.date().toString("yyyy-MM-dd")
            table.cell(4, 0).text = "Last Day Worked"
            table.cell(4, 1).text = self.last_day_edit.date().toString("yyyy-MM-dd")

            doc.add_paragraph()
            doc.add_paragraph("Income Information", style="Heading 2")
            table2 = doc.add_table(rows=3, cols=2)
            table2.cell(0, 0).text = "Insurable Earnings (52w)"
            table2.cell(0, 1).text = f"${self.insurable_earnings_spin.value():,.2f}"
            table2.cell(1, 0).text = "Insurable Hours (52w)"
            table2.cell(1, 1).text = f"{self.insurable_hours_spin.value():,.2f}"
            table2.cell(2, 0).text = "Pay Period Type"
            table2.cell(2, 1).text = self.pay_period_type_combo.currentText()
            ...
            self._write_audit_log()
            QMessageBox.information(
                self, "Success", f"✅ ROE exported to Word:\n{filename}"
            )
        except ImportError:
            QMessageBox.warning(
                self,
    ...
    def _build_document(self) -> QTextDocument:
        ...
        content.append(
            f"<p><b>Employee:</b> {self.employee_name_edit.text()}"
            f"({self.employee_number_edit.text()})</p>"
        )
        content.append(f"<p><b>SIN:</b> {self.employee_sin_edit.text()}</p>")
        content.append(
            f"<p><b>Reason:</b> {self.reason_combo.currentText()}</p>"
        )
        content.append(
            f"<p><b>Last Day Worked:</b>"
            f"{self.last_day_edit.date().toString('yyyy-MM-dd')}</p>"
        )
        content.append(
            f"<p><b>Pay Period Type:</b>"
            f"{self.pay_period_type_combo.currentText()}</p>"
        )
        content.append(
            f"<p><b>Insurable Hours (52w):</b>"
            f"{self.insurable_hours_spin.value():,.2f}</p>"
        )
        content.append(
            f"<p><b>Insurable Earnings (52w):</b>"
            f"${self.insurable_earnings_spin.value():,.2f}</p>"
        )
        content.append(
            f"<p><b>Last Pay Period:</b>"
            f"{self.last_pay_period_start.date().toString('yyyy-MM-dd')} →"
            f"{self.last_pay_period_end.date().toString('yyyy-MM-dd')}</p>"
        )
    ...
    def _write_audit_log(self) -> None:
        try:
            ...
            with open(log_path, "a", encoding="utf-8") as f:
                last_day = self.last_day_edit.date().toString("yyyy-MM-dd")
                employee = self.employee_name_edit.text()
                reason = self.reason_combo.currentText()
                hours = self.insurable_hours_spin.value()
                earnings = self.insurable_earnings_spin.value()
                log_line = (
