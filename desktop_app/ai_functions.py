...
class AIFunctionRegistry:
    ...
    def __init__(
        ...
        self.db_port = int(
            ...
            or os.environ.get("LOCAL_DB_PORT")
            or 5432
        )
        ...
        sslmode = (
            ...
            or os.environ.get("LOCAL_DB_SSLMODE")
            or ""
        )
        self.db_sslmode = sslmode.strip() or None
        self.conn = None
        self._connect()
        ...
        try:
            ...
            try:
                self.conn = psycopg2.connect(
                    ...
                    user=self.db_user,
                    sslmode=self.db_sslmode or "prefer",
                )
                return
    ...
    def get_trial_balance(self, year: int, month: int) -> dict[str, Any]:
        ...
        try:
            ...
            cur.execute(
                ...
                SELECT
                    account as account,
                    SUM(CASE WHEN debit IS NOT NULL THEN debit ELSE 0 END) as
                    debit,
    ...
    def get_monthly_summary(self, year: int, month: int) -> dict[str, Any]:
        ...
        try:
            ...
            payment_row = cur.fetchone() or {}
            charter_count = payment_row.get("charter_count") or 0
            payment_count = payment_row.get("payment_count") or 0
            total_revenue = Decimal(str(payment_row.get("total_revenue") or 0))

            # Expenses from receipts; prefer gross_amount on the live schema.
            cur.execute(
                """
                SELECT
            ...
            # Approximate GST on net collected Alberta charter revenue.
    ...
    def calculate_wcb_owed(
        ...
        try:
            ...
            if not payroll_row:
                return {
                    "error": "No payroll data found for specified date range"
                }
            ...
            return {"error": f"WCB calculation failed: {e!s}"}

    def calculate_employee_pay(
        self, employee_id: int, period_id: int = None
    ) -> dict[str, Any]:
        """
        Calculate employee pay for a specific period with all deductions.

        Formula: (charter_hours × hourly_rate) + gratuity - (CPP + EI +
        ...
        try:
            ...
            if not pay:
                return {
                    "error": f"No pay records found for employee {employee_id}"
                }
    ...
    def check_missing_deductions(self, year: int) -> dict[str, Any]:
        ...
        try:
            ...
            for row in cur.fetchone() if cur else []:
                ...
                suggested = self._suggest_category(
                    vendor, row["payment_method"], row["notes"]
                )
            ...
            uncategorized_percent = (
                (len(uncategorized) / total_count * 100)
                if total_count > 0
                else 0
            )
    ...
    def get_unpaid_charters(
        self, start_date: str, end_date: str
    ) -> dict[str, Any]:
        """
        Find all charters with outstanding balance due.

        Used for accounts receivable aging, collection follow-up,
        ...
        try:
            ...
            for row in cur.fetchall():
                ...
                balance_owing = (
                    Decimal(str(row["total_amount_due"] or 0)) - balance
                )
    ...
    def update_lms_data(
        self, reserve_number: str, field: str, value: Any
    ) -> dict[str, Any]:
        """
        Update legacy LMS data in charters table.

        CAUTION: This modifies historical data. Requires admin permission and
        confirmation.
        ...
        Returns:
            {
                ...
                "message": str}
        """
        # Strict allowlist + schema validation for dynamic field updates
        ...
        try:
            ...
            if not bool(cur.fetchone()["exists"]):
                return {
                    "success": False,
                    "error": f"Field '{field_name}' does not exist on"
                    "charters table",
                }
            ...
            if not row:
                return {
                    "success": False,
                    "error": f"Charter {reserve_number} not found",
                }
            ...
            return {
                ...
                "old_value": old_value,
                "new_value": value,
                "message": f"Updated {reserve_number}.{field_name} from"
                "'{old_value}' to '{value}'",
            }
    ...
    def _suggest_category(
        self, vendor: str, payment_method: str = None, notes: str = None
    ) -> str:
        ...
        if any(
            x in vendor_lower
            for x in ["esso", "shell", "petro canada", "co-op", "gas", "fuel"]
        ):
            return "Vehicle Fuel"
        ...
        if any(
            x in vendor_lower
            for x in ["staples", "office depot", "amazon", "supplies"]
        ):
        ...
        if any(
            x in vendor_lower
            for x in ["restaurant", "cafe", "mcdonald", "subway", "pizza"]
        ):
        ...
        if any(
            x in vendor_lower
            for x in ["rent", "lease", "landlord", "property"]
        ):
...
def calculate_employee_pay(
    employee_id: int, period_id: int = None
) -> dict[str, Any]:
    """Calculate employee pay - see"
    "AIFunctionRegistry.calculate_employee_pay()"""
...
def check_missing_deductions(year: int) -> dict[str, Any]:
    """Check missing deductions - see"
    "AIFunctionRegistry.check_missing_deductions()"""
