...
class WCBRateEntryWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        layout.addWidget(
            QLabel(
                "Create or edit WCB Alberta premium rates (per $100 of"
                "insurable earnings)."
            )
        )
    ...
    def load_rates(self) -> None:
        try:
            with DatabaseContext(self.db, auto_commit=False) as cur:
                cur.execute("""
                    SELECT year, industry_code, base_premium_rate_per_100,
                    experience_adjusted_rate_per_100, description
            ...
            logger.error(f"Failed to load rates: {exc}")
            self._set_status(f"Failed to load rates: {exc}", error=True)

    def _load_from_table(self, row, column) -> None:
    ...
    def load_entry(self) -> None:
        ...
        if not code:
            QMessageBox.warning(
                self, "Missing Code", "Enter an industry code."
            )
            return
        ...
        try:
            with DatabaseContext(self.db, auto_commit=False) as cur:
                cur.execute(
                    """
                    SELECT base_premium_rate_per_100,
                    experience_adjusted_rate_per_100, description
                    FROM wcb_ab_premium_rates
            ...
            if not row:
                ...
                self._set_status(
                    "No record found. Enter details and Save to create."
                )
                return
            ...
            logger.error(f"Failed to load entry: {exc}")
            ...
            with DatabaseContext(self.db, auto_commit=True) as cur:
                cur.execute(
                    ...
                    INSERT INTO wcb_ab_premium_rates (
                        year, industry_code, description,
                        base_premium_rate_per_100,
                        experience_adjusted_rate_per_100) VALUES (%s, %s, %s,
                        %s, %s)
                    ON CONFLICT (year, industry_code) DO UPDATE SET
                        description = EXCLUDED.description,
                        base_premium_rate_per_100 =
                        EXCLUDED.base_premium_rate_per_100,
                        experience_adjusted_rate_per_100 =
                        EXCLUDED.experience_adjusted_rate_per_100
                    """,
                    (
                        year,
                        code,
    ...
    def delete_entry(self) -> None:
        ...
        try:
            with DatabaseContext(self.db, auto_commit=True) as cur:
                cur.execute(
                    "DELETE FROM wcb_ab_premium_rates WHERE year = %s AND "
                    "industry_code = %s",
                    (year, code),
                )
