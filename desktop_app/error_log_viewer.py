...
class ErrorLogViewer(QWidget):
    ...
    def load_errors(self) -> None:
        ...
        try:
            ...
            # Get errors
            errors = self.error_logger.get_recent_errors(
                limit=500, resolved=resolved
            )
            ...
            for row_idx, error in enumerate(errors):
                ...
                self.errors_table.setItem(
                    row_idx, 0, QTableWidgetItem(str(error_id))
                )
                self.errors_table.setItem(
                    row_idx, 1, QTableWidgetItem(str(timestamp))
                )
                self.errors_table.setItem(
                    row_idx, 2, QTableWidgetItem(error_type)
                )
                ...
                self.errors_table.setItem(
                    row_idx, 3, QTableWidgetItem(msg_short)
                )

                self.errors_table.setItem(
                    row_idx, 4, QTableWidgetItem(widget_name)
                )
    ...
    def view_selected_error(self) -> None:
        ...
        if current_row < 0:
            QMessageBox.warning(
                self, "No Selection", "Please select an error to view."
            )
            return
    ...
    def resolve_selected_error(self) -> None:
        """Mark selected error as resolved"""
        current_row = self.errors_table.currentRow()
        if current_row < 0:
            QMessageBox.warning(
                self, "No Selection", "Please select an error to resolve."
            )
            return
        ...
        notes, ok = QInputDialog.getMultiLineText(
            self, "Resolution Notes", "Enter resolution notes (optional):"
        )

        if ok:
            if self.error_logger.mark_resolved(error_id, notes):
                QMessageBox.information(
                    self, "Success", "Error marked as resolved."
                )
                self.load_errors()
            else:
                QMessageBox.critical(
                    self, "Error", "Failed to mark error as resolved."
                )
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(
                    self,
                    "Success",
                    f"Deleted {deleted_count} resolved errors.",
                )
                self.load_errors()
            except Exception as e:
                QMessageBox.critical(
                    self, "Error", f"Failed to delete errors: {e}"
                )
    ...
    def export_errors(self) -> None:
        ...
        try:
            ...
            filename = str(_APP_ROOT / "reports" / f"error_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
            ...
            QMessageBox.information(
                self, "Success", f"Errors exported to:\n{filename}"
            )
        except Exception as e:
            QMessageBox.critical(
                self, "Error", f"Failed to export errors: {e}"
            )
...
class ErrorDetailDialog(QDialog):
    ...
    def __init__(self, error_details, parent=None) -> None:
        super().__init__(parent)
    ...
    def init_ui(self) -> None:
        ...
        info_layout.addRow(
            "Status:", QLabel("✅ Resolved" if resolved else "❌ Unresolved")
        )
