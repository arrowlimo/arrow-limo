...
class DocumentManagementWidget(QWidget):
    ...
    def init_ui(self) -> None:
        ...
        for category in self.categories:
            self.tabs.addTab(
                self._create_category_tab(category), f"📋 {category}"
            )
    ...
    def _create_category_tab(self, category) -> object:
        ...
        table.setHorizontalHeaderLabels(
            ["Title", "Category", "Upload Date", "Size", "Tags", "Actions"]
        )
    ...
    def load_documents(self) -> None:
        ...
        try:
            ...
            logger.error("Documents table not found: %s", e)
    ...
    def upload_document(self) -> None:
        ...
        if not self.doc_title.text().strip():
            QMessageBox.warning(
                self, "Missing Title", "Please enter a document title."
            )
            return
        ...
        files, _ = QFileDialog.getOpenFileNames(
            ...
            "",
            "All Files (*);;PDF Files (*.pdf);;Images (*.jpg *.jpeg"
            "*.png);;Word (*.doc *.docx);;Excel (*.xls *.xlsx)",
        )
        ...
        if files:
            ...
            QMessageBox.information(
                self, "Success", f"Uploaded {len(files)} document(s)!"
            )
    ...
    def view_document(self, table) -> None:
        """View selected document"""
        selected = table.selectedItems()
        if not selected:
            QMessageBox.warning(
                self, "No Selection", "Please select a document."
            )
            return
