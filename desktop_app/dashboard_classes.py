...
class FleetManagementWidget(BaseReportWidget):
    """Fleet management dashboard - vehicle costs and maintenance with action"
    "buttons"""

    def __init__(self, db) -> None:
        columns = [
            {"header": "Vehicle", "key": "vehicle"},
            {"header": "Make/Model", "key": "make_model"},
            {"header": "Year", "key": "year"},
            {"header": "Fuel $", "key": "fuel_$"},
            {"header": "Maint $", "key": "maint_$"},
            {"header": "Total $", "key": "total_$"},
        ]
        super().__init__(db, "FleetManagement", columns)
        self.db = db
    ...
    def load_data(self) -> None:
        try:
            ...
            with DatabaseContext(self.db, auto_commit=False) as cur:
                cur.execute(
                    """SELECT v.vehicle_id, v.vehicle_number, v.make, v.model,
                    v.year,
                    CASE
                    ...
                    v.year, v.operational_status, v.decommission_date,
                    v.last_service_date, v.next_service_due
            ...
            for i, (
                ...
                self.table.setItem(
                    i,
                    1,
                    QTableWidgetItem(f"{make or ''} {model or ''}".strip()),
                )
                self.table.setItem(i, 2, QTableWidgetItem(str(year or "")))
                self.table.setItem(
                    i, 3, QTableWidgetItem(status or "✅ Active")
                )
                self.table.setItem(
                    i, 4, QTableWidgetItem(str(last_service or ""))
                )
                self.table.setItem(
                    i, 5, QTableWidgetItem(str(next_service or ""))
                )
                self.table.setItem(
                    i, 6, QTableWidgetItem(f"${float(fuel or 0):,.2f}")
                )
                self.table.setItem(
                    i, 7, QTableWidgetItem(f"${float(maint or 0):,.2f}")
                )
                self.table.setItem(
                    i,
                    8,
                    QTableWidgetItem(
                        f"${float(fuel or 0) + float(maint or 0):,.2f}"
                    ),
                )
        except Exception:
    ...
    def edit_vehicle(self) -> None:
        ...
        if current_row < 0:
            QMessageBox.warning(
                self, "Warning", "Please select a vehicle first"
            )
            return
    ...
    def retire_vehicle(self) -> None:
        ...
        reply = QMessageBox.question(
            self,
            "Confirm Retire",
            f"Retire vehicle {vehicle_num}? This will set its status to"
            f"retired.",
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(
                    self, "Success", f"Vehicle {vehicle_num} retired ✅"
                )
                self.load_data()
            except Exception as e:
                logger.error(f"Failed to retire vehicle: {e}")
                QMessageBox.critical(
                    self, "Error", f"Failed to retire vehicle: {e}"
                )
    ...
    def mark_inactive(self) -> None:
        ...
        reply = QMessageBox.question(
            ...
            f"Mark vehicle {vehicle_num} as inactive? It can be reactivated"
            f"later.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        )
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(
                    self,
                    "Success",
                    f"Vehicle {vehicle_num} marked inactive ✅",
                )
                self.load_data()
            except Exception as e:
                logger.error(f"Failed to mark vehicle inactive: {e}")
                QMessageBox.critical(
                    self, "Error", f"Failed to mark vehicle inactive: {e}"
                )


class DriverPerformanceWidget(BaseReportWidget):
    ...
    def __init__(self, db) -> None:
        ...
        super().__init__(db, "DriverPerformance", columns)
        self.db = db
        layout = QVBoxLayout()
        layout.addWidget(
            QLabel(
                "<h3>👤 Driver Performance</h3><p>YTD earnings and charter"
                "count</p>"
            )
        )
        self.table = QTableWidget()
    ...
    def load_data(self) -> None:
        try:
            rows = self.db.safe_query("""SELECT e.full_name,
                ...
                ORDER BY gross DESC""")
            print(f"[OK] Driver Performance loaded {len(rows)} drivers")
            self.table.setRowCount(len(rows))
            for i, (name, charters, gross, ded, net) in enumerate(rows):
                self.table.setItem(i, 0, QTableWidgetItem(str(name or "")))
                self.table.setItem(
                    i, 1, QTableWidgetItem(str(int(charters or 0)))
                )
                self.table.setItem(
                    i, 2, QTableWidgetItem(f"${float(gross or 0):,.2f}")
                )
                self.table.setItem(
                    i, 3, QTableWidgetItem(f"${float(ded or 0):,.2f}")
                )
                self.table.setItem(
                    i, 4, QTableWidgetItem(f"${float(net or 0):,.2f}")
                )
        except Exception:
            logger.exception("Driver Performance load error")


class FinancialDashboardWidget(BaseReportWidget):
    ...
    def __init__(self, db) -> None:
        ...
        super().__init__(db, "FinancialDashboard", columns)
        self.db = db
        layout = QVBoxLayout()
    ...
    def load_data(self) -> None:
        try:
            if not hasattr(self, "revenue_label"):
                return
            revenue = self.db.safe_scalar(
                "SELECT SUM(total_amount_due) FROM charters"
            )
            expense = self.db.safe_scalar(
                "SELECT SUM(gross_amount) FROM receipts WHERE category !="
                "'personal' AND gross_amount IS NOT NULL"
            )
            print(
                f"[OK] Financial Dashboard: Revenue ${revenue:,.2f}, Expenses"
                f"${expense:,.2f}"
            )
            ...
            logger.exception("Financial Dashboard load error")
...
class PaymentReconciliationWidget(BaseReportWidget):
    ...
    def __init__(self, db) -> None:
        columns = [
            ...
            {"header": "Charter Date", "key": "charter_date"},
            {
                "header": "Balance",
                "key": "balance",
                "format": lambda v: f"${float(v or 0):,.2f}",
            },
    ...
    def fetch_rows(self) -> object:
        try:
            with DatabaseContext(self.db, auto_commit=False) as cur:
                cur.execute("""
                    SELECT c.reserve_number, cl.company_name, c.charter_date,
                    c.balance,
                ...
                rows = cur.fetchall()
            print(
                f"[OK] Payment Reconciliation loaded {len(rows)} outstanding"
                f"charters"
            )
            return [
                {
            ...
            logger.exception("Payment Reconciliation load error")
            raise e
