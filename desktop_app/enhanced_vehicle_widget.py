...
class EnhancedVehicleListWidget(QWidget):
    ...
    - Columns: Vehicle #, Plate, Make/Model, Year, Type, Mileage, Status,
    Next Service, Issues
    ...
    def __init__(self, db, parent=None) -> None:
        ...
        self.status_filter.addItems(
            ["All", "Active", "In Service", "Out of Service", "Retired"]
        )
        ...
        try:
            with DatabaseContext(self.db, auto_commit=False) as cur:
                ...
                if self.status_filter.currentText() != "All":
                    ...
                    query += (
                        " AND COALESCE(v.operational_status, 'active') = %s"
                    )
            ...
            if rows:
                for i, (
                    ...
                    self.table.setItem(
                        i, 2, QTableWidgetItem(str(plate or ""))
                    )
                    self.table.setItem(
                        i, 3, QTableWidgetItem(str(make_model or ""))
                    )
                    self.table.setItem(i, 4, QTableWidgetItem(str(yr or "")))
                    self.table.setItem(
                        i, 5, QTableWidgetItem(str(vtype or ""))
                    )
                    self.table.setItem(
                        i, 6, QTableWidgetItem(f"{int(mileage or 0):,}")
                    )
                    self.table.setItem(
                        i, 7, QTableWidgetItem(str(status or "Active"))
                    )
                    self.table.setItem(
                        i, 8, QTableWidgetItem(str(next_svc or "N/A"))
                    )
                    self.table.setItem(
                        i, 9, QTableWidgetItem(str(alerts or 0))
                    )

                    # Visual alerts
                    # Placeholder: In real implementation, check next_service
                    # vs today
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to load vehicles: {e}"
            )
    ...
    def mark_inactive(self) -> None:
        ...
        if current_row >= 0:
            ...
            reply = QMessageBox.question(
                ...
                f"Mark vehicle {vehicle_num} as inactive? It can be"
                f"reactivated later.",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            )
            if reply == QMessageBox.StandardButton.Yes:
