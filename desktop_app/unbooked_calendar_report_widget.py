...
class UnbookedCalendarReportWidget(QWidget):
    ...
    # Emits event data when booking is created
    ...
    def load_unbooked_events(self) -> None:
        ...
        try:
            ...
            try:
                with DatabaseContext(self.db, auto_commit=False) as cur:
                    if not table_exists(self.db, "calendar_events"):
                        self.unbooked_events = []
                        self.display_events()
                        self.status_label.setText("No calendar events table")
                        return
                    ...
                    cur.execute(
                        """
                        SELECT
                            id, event_date, event_time, event_title,
                            driver_name, vehicle_type, event_notes
                    ...
                    for event in all_events:
                        ...
                        # Fuzzy match title to client, then check if charter
                        # exists
                        client_id = self.fuzzy_match_client(title)

                        if client_id:
                            # Check if charter exists for this date with this
                            # client
                            cur.execute(
                                """
                ...
                # calendar_events table may not exist - silent fallback
            ...
            logger.error(f"Error in load_unbooked_events: {e}")

    def fuzzy_match_client(self, event_title) -> object:
        """Fuzzy match event title to a client"""
        try:
            if not event_title:
                return None

            event_title_text = str(event_title).strip()
            if not event_title_text:
                return None

            with DatabaseContext(self.db, auto_commit=False) as cur:
                cur.execute(
                    "SELECT client_id, client_name FROM clients LIMIT 500"
                )
            ...
            event_title_lower = event_title_text.lower()

            for client_id, client_name in clients:
                if not client_name:
                    continue
                ratio = SequenceMatcher(
                    None, event_title_lower, str(client_name).lower()
                ).ratio()
            ...
            logger.error(f"Error fuzzy matching client: {e}")
            return None

    def get_client_name(self, client_id) -> object:
    ...
    def display_events(self) -> None:
        ...
        for row, event in enumerate(self.unbooked_events):
            ...
            if event["client_id"]:
                # Light green if matched
                title_item.setBackground(QColor(200, 255, 200))
            self.table.setItem(row, 2, title_item)

            # Driver
            driver_item = QTableWidgetItem(event["driver"] or "")
            driver_item.setFlags(
                driver_item.flags() & ~Qt.ItemFlag.ItemIsEditable
            )
            self.table.setItem(row, 3, driver_item)

            # Vehicle
            vehicle_item = QTableWidgetItem(event["vehicle"] or "")
            vehicle_item.setFlags(
                vehicle_item.flags() & ~Qt.ItemFlag.ItemIsEditable
            )
            ...
            notes_item.setFlags(
                notes_item.flags() & ~Qt.ItemFlag.ItemIsEditable
            )
        ...
        self.status_label.setText(
            f"Showing {len(self.unbooked_events)} unbooked events"
        )
        self.table.resizeRowsToContents()

    def book_event(self, row) -> None:
        ...
        QMessageBox.information(
            self,
            "Success",
            f"Opening new charter form for {event['client_name']} on"
