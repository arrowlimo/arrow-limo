...
WHERE COALESCE(debit_amount,0)>0
  ...
  AND (
    COALESCE(vendor_extracted,'') ILIKE '%HEFFNER LEXUS%'
    OR COALESCE(vendor_extracted,'') ILIKE '%HEFFNER AUTO FINANCE%'
    OR COALESCE(vendor_extracted,'') ILIKE '%HEFFNER%'
    OR COALESCE(vendor_extracted,'') ILIKE '%TELUS%'
    OR COALESCE(vendor_extracted,'') ILIKE '%ALL SERVICE INSURANCE%'
    OR COALESCE(vendor_extracted,'') ILIKE '%ASI INSURANCE%'
    OR COALESCE(vendor_extracted,'') ILIKE '%ASI FINANCE%'
  )
""")
print('remaining_target_vendor_unlinked_debits:', dict(cur.fetchone()))
