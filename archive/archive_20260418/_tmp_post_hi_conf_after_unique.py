...
for x in a:
    ...
    for y in b:
        ...
        if ov<2: continue
        cur.execute("SELECT COUNT(*) AS c FROM receipts WHERE banking_transaction_id=%s", (x['transaction_id'],)); r8362=cur.fetchone()['c']
        cur.execute("SELECT COUNT(*) AS c FROM receipts WHERE banking_transaction_id=%s", (y['transaction_id'],)); r1615=cur.fetchone()['c']
        pairs.append((x['transaction_id'],y['transaction_id'],x['transaction_date'].year,float(x['debit_amount']),dd,ov,r8362,r1615,x['description'],y['description']))

print('remaining_hi_conf_ov2_pairs', len(pairs))
print('remaining_hi_conf_where_8362_has_receipt', sum(1 for p in pairs if p[6]>0))
for p in pairs[:80]:
    print({'tx8362':p[0],'tx1615':p[1],'year':p[2],'amt':p[3],'day_gap':p[4],'ov':p[5],'r8362':p[6],'r1615':p[7],'desc8362':p[8],'desc1615':p[9]})
