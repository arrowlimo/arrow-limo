...
# Keep only where at least one receipt is linked to either tx
apply_pairs=[]
...
print('safe_pairs_total', len(safe))
...
for x,y,best,rs in apply_pairs:
    ...
    # move explicit rbl for keep receipt if needed
    ...
    report.append((tx_bad, tx_good, keep_rid, unlinked, moved, del_rbl, del_tx, x['transaction_date'].year))
...
print('deleted_transactions', sum(r[6] for r in report))
print('total_unlinked_receipts', sum(r[3] for r in report))
print('total_deleted_rbl', sum(r[5] for r in report))

yr=defaultdict(int)
for r in report: yr[r[7]] += 1
print('deleted_by_year', dict(sorted(yr.items())))
print('first_80_changes')
for r in report[:80]:
    print({'tx8362_deleted':r[0],'tx1615_kept':r[1],'receipt_kept':r[2],'unlinked':r[3],'moved_rbl':r[4],'deleted_rbl':r[5],'year':r[7]})

# Validate remaining strict duplicates under same criteria
