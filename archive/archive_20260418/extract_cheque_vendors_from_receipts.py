...
WHERE bt.debit_amount IS NOT NULL
  ...
  AND NOT (COALESCE(r.vendor_name, '') ~* '(^|[^A-Za-z])NSF([^A-Za-z]|$)')
  AND NOT (COALESCE(r.description, '') ~* '(^|[^A-Za-z])NSF([^A-Za-z]|$)')
  AND NOT (COALESCE(bt.description, '') ~* '(^|[^A-Za-z])NSF([^A-Za-z]|$)')
  AND COALESCE(bt.description, '') !~* 'RETURN|REVERSAL|ERROR CORRECTION'
