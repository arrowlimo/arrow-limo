...
RESERVE = '006311'
DELETE_PAYMENT_ID = 8056
TARGET_INCOME_ID = 22228
...
try:
    payment_backup = f'backup_payments_fix_006311_{TS}'
    charter_backup = f'backup_charters_fix_006311_{TS}'
    cp_backup = f'backup_charter_payments_fix_006311_{TS}'
    income_backup = f'backup_income_ledger_fix_006311_{TS}'
    ...
    cur.execute(sql.SQL('DROP TABLE IF EXISTS {}').format(sql.Identifier(income_backup)))
    cur.execute(sql.SQL('CREATE TABLE {} AS SELECT * FROM income_ledger WHERE false').format(sql.Identifier(income_backup)))
    cur.execute(sql.SQL('INSERT INTO {} SELECT * FROM income_ledger WHERE income_id = %s').format(sql.Identifier(income_backup)), (TARGET_INCOME_ID,))
    ...
    cur.execute('UPDATE income_ledger SET payment_id = NULL WHERE income_id = %s', (TARGET_INCOME_ID,))
    ...
    cur.execute(
        ...
        SELECT
            ...
            p.payment_method,
            p.payment_key,
            'PAYMENTS_TABLE_REBUILD_20260417_AFTER_006311_FIX'
        FROM payments p
    ...
    print(f'income_backup={income_backup}')
    print(f'charter_backup={charter_backup}')
    print(f'charter_payments_backup={cp_backup}')
    print('deleted_payment_id=8056')
    print('nulled_income_id=22228')
except Exception:
    conn.rollback()
    raise
finally:
