...
def calculate_charges(vehicle_type: str, start_time: str, end_time: str, has_airport: bool = False):
    """Simulate _update_invoice_charges logic"""
    ...
    if pricing.get("nrr", 0.0) > 0:
        return charges, f"NRR > 0: {pricing['nrr']}"
    ...
    # Gratuity (18% of charter)
    if hourly_rate > 0 and total_hours > 0:
        charter_total = hourly_rate * total_hours
        gratuity = charter_total * 0.18
        charges.append(("Gratuity", "Percent", f"${gratuity:.2f}"))
    
    return charges, f"✅ {total_hours:.2f} hours"

# Test 1: Sedan (common vehicle type)
print("=" * 60)
print("TEST 1: Sedan 3-hour charter (08:00 - 11:00)")
print("=" * 60)
vehicle_type = "Sedan (3-4 pax)"
charges, info = calculate_charges(vehicle_type, "08:00", "11:00")
print(f"Vehicle: {vehicle_type}")
print(f"Duration: {info}")
pricing = load_pricing_defaults(vehicle_type)
print(f"Pricing: Hourly ${pricing['hourly_rate']}, Standby ${pricing['standby_rate']}, NRR {pricing['nrr']}")
print(f"Charges:")
...
print("\n" + "=" * 60)
print("TEST 2: SUV 2-hour charter with airport (14:00 - 16:00)")
print("=" * 60)
vehicle_type = "Luxury SUV (3-4 pax)"
charges, info = calculate_charges(vehicle_type, "14:00", "16:00", has_airport=True)
print(f"Vehicle: {vehicle_type}")
print(f"Duration: {info}")
pricing = load_pricing_defaults(vehicle_type)
print(f"Pricing: Hourly ${pricing['hourly_rate']}, Airport CY ${pricing['airport_pickup_calgary']}, NRR {pricing['nrr']}")
print(f"Charges:")
for desc, calc_type, total in charges:
    print(f"  - {desc:20} | {calc_type:8} | {total}")

# Test 3: Check available vehicle types
print("\n" + "=" * 60)
print("TEST 3: Available vehicle types in DB")
print("=" * 60)
try:
    conn = psycopg2.connect(host=DB_HOST, database=DB_NAME, user=DB_USER, password=DB_PASSWORD)
    cur = conn.cursor()
    ...
    for vtype, hrly, standby, nrr in rows:
        ...
        print(f"  - {vtype:15} Hourly: ${hrly:8.2f}  Standby: ${standby:8.2f}  {nrr_flag}")
except Exception as e:
    print(f"❌ Error fetching vehicle types: {e}")

print("\n✅ Auto-charges verification complete")
