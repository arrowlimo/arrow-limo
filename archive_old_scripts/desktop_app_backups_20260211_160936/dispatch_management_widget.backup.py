...
class DispatchManagementWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.status_filter.addItems(
            ["All", "Pending", "Assigned", "Active", "Completed"])
        ...
        self.bookings_table.horizontalHeader(
        ).customContextMenuRequested.connect(self.show_column_menu)
    ...
    def load_bookings(self):
        ...
        try:
            ...
            try:
                ...
                pass
            QMessageBox.warning(
    self, "Load Error", f"Failed to load bookings: {e}")
    ...
    def filter_bookings(self):
        ...
        for booking in self.bookings_data:
            ...
            # Apply date filter (optional - include all if no specific date
            # selected)
    ...
    def handle_double_click(self, item):
        ...
        row = item.row()
        booking = self.bookings_data[row] if row < len(
            self.bookings_data) else None

        if booking:
            # reserve_number is second column in query
     ...
     "Please select a booking first.")
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                cur = self.db.get_cursor()
                cur.execute(
    "DELETE FROM charters WHERE charter_id = %s", (self.current_charter_id,))
                self.db.commit()
    ...
    self,
    "New Booking",
     "Booking creation will be implemented via modal form.")
    ...
    self,
    "Missing Reserve #",
     "Selected booking has no reserve number.")
            return

        try:
            dlg = CharterDetailDialog(self.db, reserve_number=str(
                reserve_number), parent=self, initial_tab='orders')
            dlg.exec()
        except Exception as e:
            try:
                self.db.rollback()
    ...
    lambda checked,
    c=col: self.bookings_table.setColumnHidden(
        c,
         not checked))
    ...
    self,
    "View Reset",
     "Column widths and visibility reset to defaults.")
    ...
    def load_full_charter_details(self, charter_id):
        ...
        try:
            ...
            if result:
                ...
                self.detail_route_description.setPlainText(
                    str(result[10] or ""))
                ...
                self.detail_total_amount.setText(
                    f"${float(result[14] or 0.0):.2f}")
                self.detail_balance_due.setText(
                    f"${float(result[15] or 0.0):.2f}")
            ...
            for idx, (item, qty, price) in enumerate(beverages):
                self.beverage_table.setItem(
                    idx, 0, QTableWidgetItem(str(item or "")))
                self.beverage_table.setItem(
                    idx, 1, QTableWidgetItem(str(qty or "")))
                price_val = float(price or 0.0)
                self.beverage_table.setItem(
                    idx, 2, QTableWidgetItem(f"${price_val:.2f}"))
                bev_total += price_val
    ...
    def insert_route_between(self):
        """Insert a new route/stop between pickup and dropo"""
        dialog = QDialog(self)
    ...
    self,
    "Route Updated",
     "Route has been updated. Click 'Save Changes' to commit.")
            else:
                QMessageBox.warning(
    self, "Empty Location", "Please enter a location.")
