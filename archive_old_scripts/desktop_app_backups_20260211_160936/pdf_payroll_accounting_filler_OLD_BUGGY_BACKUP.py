...
class PayrollAccountingPDFFiller:
    ...
    month: int,
     output_path: str = None) -> Optional[str]:
        ...
        try:
            ...
            if not payroll:
                print(
                    f"No payroll data for employee {employee_id} in {year}-{month:02d}")
                return None
    ...
    end_date: date,
     output_path: str = None) -> Optional[str]:
        ...
        try:
            ...
            # Expenses table
            exp_data = [["Date", "Vendor", "Description",
                "Amount", "GST", "Category"]]
            total_amt = Decimal('0')
            total_gst = Decimal('0')
            ...
            exp_data.append(
                ["", "", "TOTAL:", f"${total_amt:,.2f}", f"${total_gst:,.2f}", ""])
    ...
    vendor_name: str,
     output_path: str = None) -> Optional[str]:
        ...
        if output_path is None:
            output_path = f"vendor_statement_{
    vendor_name.replace(
        ' ', '_')}.pdf"
    ...
    Paragraph(
        f"VENDOR STATEMENT: {
            ...
            # Invoices table
            inv_data = [["Invoice #", "Date", "Amount",
                "Tax", "Status", "Paid Date"]]
            total_amt = Decimal('0')
            total_tax = Decimal('0')
