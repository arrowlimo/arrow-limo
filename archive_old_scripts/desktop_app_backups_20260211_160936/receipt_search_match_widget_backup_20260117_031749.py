...
from PyQt6.QtGui import QColor, QDoubleValidator, QFont, QGuiApplication, QPainter
...
class ReceiptCompactDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option, index):
        ...
        amount_str = f"${
    amount:,.2f}" if isinstance(
        amount, (int, float)) else (
...
class ReceiptSearchMatchWidget(QWidget):
    ...
    def _build_search_panel(self) -> QWidget:
        ...
        self.vendor_filter.setPlaceholderText(
            "Type vendor name (fuzzy match, e.g., 'Fibr' finds Fibrenew)")
        ...
        self.find_banking_btn.clicked.connect(
            self._search_banking_transactions)
        ...
        self.results_label.setStyleSheet(
            "color: #666; font-size: 8pt; margin-top: 3px;")
    ...
    self,
    "Banking Search",
     "Banking transaction search coming soon.")
    ...
    def _build_detail_panel(self) -> QWidget:
        ...
        self.gst_override_reason.addItems(
            ["Manual - Government fee", "Manual - NGO donation", "Manual - test"])
        ...
        self.override_note.setPlaceholderText(
            "Optional note for audit (e.g., gov fee exempt, only service fee taxable)")
        ...
        self.invoice_number.setPlaceholderText(
            "e.g., INV-2023-001 or leave blank for auto")
        ...
        try:
            ...
            completer = QCompleter([self.new_gl.itemText(i)
    ...
    lambda: QGuiApplication.clipboard().setText(
        ...
        # Banking tip
        tip_label = QLabel(
            "💡 TIP: Multiple receipts can share the same Banking Transaction ID (e.g., one $2,772 deposit paying 3 invoices)")
    ...
    QLabel("Format: 10 (=$10.00), 10.50 or $0 (+0.50)"))
     ...
     "Select a receipt from the table to link to invoice.")
        ...
        try:
            ...
            try:
                cur = self.conn.cursor()
                cur.execute(
                    "SELECT DISTINCT vendor_name FROM vendor_invoices WHERE vendor_name IS NOT NULL ORDER BY vendor_name")
    ...
    "WHERE vendor_name = %s ORDER BY invoice_date DESC", (vendor,))
    ...
    self, "No Invoice", "Please select an invoice.")
            ...
            QMessageBox.information(
    self, "Linked", f"Receipt #{rid} linked to invoice #{
        invoice_combo.currentText()}.")
            self._do_search()
    ...
    def _link_to_charter(self):
        ...
        if not hasattr(
    self,
     "loaded_receipt_id") or not self.loaded_receipt_id:
            QMessageBox.information(
    self,
    "No Selection",
    ...
    self,
    "include_desc_chk",
     None) and self.include_desc_chk.isChecked():
                sql.append(
        ...
        if amt > 0:
            ...
            params.extend([float(amt) - float(amt_range),
    ...
    def _populate_table(self, rows: List[tuple]):
        # Table columns: ID | Date | Vendor | Amount | GL/Category | Banking ID
        # | Charter
        ...
        for r, row in enumerate(rows):
            ...
            # description/payment_method provided
    ...
    Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
            ...
            self.results_table.setItem(r, 5, QTableWidgetItem(
    ...
    self,
    "compact_toggle",
     None) and self.compact_toggle.isChecked():
    ...
    amt_item.text().replace(
        "$", "").replace(
            ...
            # selected
            self.loaded_receipt_id = int(
    rid_item.text()) if rid_item and rid_item.text().isdigit() else None
    ...
    self,
    "Writes disabled",
     "Set RECEIPT_WIDGET_WRITE_ENABLED=true to enable adding receipts.")
            return
        try:
            date = self.new_date.date().toPyDate()
            vendor = (self.new_vendor.text() or "").strip()
            # source_reference now stored in banking_id field
            source_reference = (self.new_banking_id.text() or "").strip()
            amount = self.new_amount.value()
            desc = (self.new_desc.text() or "").strip()
            gl_text = (
    self.new_gl.currentText() or "").strip() if isinstance(
        self.new_gl,
        QComboBox) else (
            self.new_gl.text() or "").strip()
            banking_id_text = (self.new_banking_id.text() or "").strip()
            banking_id = int(
                banking_id_text) if banking_id_text.isdigit() else None
            ...
            reserve_number = (
    self.new_charter_input.text() or "").strip() or None
            gst_amount = float(self.gst_override_input.value()
                               ) if self.gst_override_enable.isChecked() else None
     ...
     source_reference or None]
            ...
            if row and row[0]:
                ...
                try:
                    ...
                    pass
                QMessageBox.information(
    self, "Added", f"Receipt #{rid} added.")
                self._clear_form()
                self._do_search()
            else:
                QMessageBox.information(
    ...
    "gl_account_name = %s",
     "banking_transaction_id = %s"]
            ...
            sql = f"UPDATE receipts SET {
    ', '.join(sets)} WHERE receipt_id = %s"
            params.append(rid)
            cur.execute(sql, params)
            self.conn.commit()
            cur.close()
            try:
                after = {
                    ...
                    "payment_method": payment_method,}
                self._audit_log(
    action="update", receipt_id=rid, details={
        "before": before, "after": after})
            except Exception:
                pass
            QMessageBox.information(
    self, "Updated", f"Receipt #{rid} updated.")
            self._do_search()
        except Exception as e:
            try:
                self.conn.rollback()
    ...
    def _prefill_from_search(self):
        ...
        try:
            ...
            if row < 0:
                QMessageBox.information(
    self, "No Selection", "Select a receipt from the search results to prefill.")
                return
    ...
    self,
    "Prefilled",
     f"Form prefilled from Receipt #{rid}. Review and modify as needed.")
    ...
    self,
    "Missing amount",
     "Enter a positive Amount before splitting.")
            return
        dlg = QDialog(self)
        dlg.setWindowTitle("Split/Allocate Receipt Amount")
        v = QVBoxLayout(dlg)
        v.addWidget(QLabel(
            "Enter one allocation per line: amount, GL/category, optional description"))
        edit = QPlainTextEdit()
    ...
    QDialogButtonBox.StandardButton.Ok | QDialogButtonBox.StandardButton.Cancel)
        ...
        for line in lines:
            ...
            try:
                ...
                QMessageBox.warning(
    self, "Parse error", f"Could not parse amount in line: {line}")
                return
        ...
        if abs(float(total) - float(base_amount)) > 0.01:
            QMessageBox.warning(
    self, "Total mismatch", f"Allocations total ${
        float(total):,.2f} does not equal current Amount ${
            float(base_amount):,.2f}.")
            return
        # Perform inserts for each allocation
        inserted = 0
        for amt, gl_text, desc in allocations:
            try:
                self._insert_allocation_line(
    amount=amt, gl_text=gl_text, extra_desc=desc)
                inserted += 1
            except Exception as e:
                try:
                    self.conn.rollback()
                    ...
                    pass
                QMessageBox.critical(
    self, "Split Error", f"Failed to insert allocation line (${
        float(amt):,.2f}, {gl_text}):\n\n{e}")
                return
        QMessageBox.information(
    self,
    "Split complete",
     f"Inserted {inserted} allocation lines.")
        self._clear_form()
        self._do_search()

    def _insert_allocation_line(
    self,
    amount: Decimal,
    gl_text: str,
     extra_desc: str):
        ...
        full_desc = (base_desc +
     (f"; {extra_desc}" if extra_desc else "")).strip() or None
        banking_id_text = (self.new_banking_id.text() or "").strip()
        banking_id = int(
            banking_id_text) if banking_id_text.isdigit() else None
        vehicle_id = self.new_vehicle_combo.currentData() if self.new_vehicle_combo.currentData() else None
        ...
        if not vendor or amount <= 0:
            raise ValueError(
                "Vendor and positive Amount are required for allocation line.")
    ...
    def _load_drivers_into_combo(self):
        try:
            cur = self.conn.cursor()
            cur.execute(
                "SELECT employee_id, CONCAT_WS(' ', first_name, last_name) FROM employees ORDER BY first_name, last_name")
            names = []
    ...
    def _attach_vendor_completer(self):
        try:
            cur = self.conn.cursor()
            cur.execute(
                "SELECT DISTINCT vendor_name FROM receipts WHERE vendor_name IS NOT NULL ORDER BY vendor_name LIMIT 5000")
    ...
    self,
    "Missing reserve #",
     "Enter a reserve number to link.")
            return
        try:
            cur = self.conn.cursor()
            cur.execute(
    "UPDATE receipts SET reserve_number = %s WHERE receipt_id = %s",
    (reserve_num,
     rid))
            self.conn.commit()
            cur.close()
            QMessageBox.information(
    self, "Linked", f"Receipt {rid} linked to Charter {reserve_num}.")
    ...
    "Missing data",
     "Enter Amount and Date to find banking matches.")
        ...
        try:
            ...
            if not rows:
                QMessageBox.information(
    self, "No matches", f"No banking transactions found matching ${
        amt:,.2f} near {date}.")
                return
            dlg = QDialog(self)
    ...
    QDialogButtonBox.StandardButton.Ok | QDialogButtonBox.StandardButton.Cancel)
            ...
            QMessageBox.information(
    self, "Selected", f"Banking Transaction {selected} selected.")
        except Exception as e:
            QMessageBox.critical(
    self,
    "Suggestion Error",
     f"Could not find banking matches:\n\n{e}")
            ...
            for row_idx, row in enumerate(rows):
                try:
                    ...
                    if "reserve_number" in row:
                        reserve_number = (
    row["reserve_number"] or "").strip() or None
    ...
    self,
    "Writes disabled",
     "Set RECEIPT_WIDGET_WRITE_ENABLED=true to enable reconciliation.")
            return
        dlg = QDialog(self)
        ...
        try:
            ...
            v.addWidget(
    QLabel(
        f"🧾 Unmatched Receipts ({
            len(unmatched_receipts)}) | 🏦 Unmatched Banking ({
                len(unmatched_banking)})"))
            # Side-by-side tables
            h = QHBoxLayout()
            # Receipts table
            ...
            for r, (rid, rdate, vendor, amt, desc) in enumerate(
                ...
                receipts_table.setItem(r, 3, QTableWidgetItem(
            ...
            for r, (bid, bdate, bdesc, bamt) in enumerate(unmatched_banking):
                ...
                banking_table.setItem(r, 3, QTableWidgetItem(
                    f"${bamt:,.2f}" if bamt else ""))
            ...
            def do_match():
                ...
                if r_row < 0 or b_row < 0:
                    QMessageBox.information(
    dlg, "Select both", "Select one receipt and one banking transaction.")
                    return
                ...
                try:
                    ...
                    cur2.execute(
    "UPDATE receipts SET banking_transaction_id = %s WHERE receipt_id = %s", (bid, rid))
                    self.conn.commit()
                    cur2.close()
                    QMessageBox.information(
    dlg, "Matched", f"Receipt {rid} linked to Banking {bid}.")
                    dlg.close()
                except Exception as e:
                    try:
                        self.conn.rollback()
                        ...
                        pass
                    QMessageBox.critical(
    dlg, "Match Error", f"Could not link receipt:\n\n{e}")
