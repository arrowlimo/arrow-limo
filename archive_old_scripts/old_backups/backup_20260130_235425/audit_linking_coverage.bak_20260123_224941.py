...
def fetch_coverage(cur):
    ...
    cur.execute("SELECT COUNT(*) FROM payments WHERE charter_id IS NOT NULL")
    ...
    cur.execute("SELECT COUNT(*) FROM payments WHERE charter_id IS NOT NULL AND payment_date >= CURRENT_DATE - INTERVAL '%s days'" % LOOKBACK_DAYS)
