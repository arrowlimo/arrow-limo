...
class LoginDialog(QDialog):
    ...
    def _on_db_target_changed(self, target: str) -> None:
        ...
        if target == "local":
            ...
            local_pwd = os.getenv("LOCAL_DB_PASSWORD") or os.getenv("DB_PASSWORD") or ""
            os.environ["DB_PASSWORD"] = local_pwd
            os.environ["DB_SSLMODE"] = ""
            
            # Do NOT auto-fill user credentials - user password is separate from DB password!
    ...
    def handle_login(self):
        ...
        # Ensure DB password exists for local target if required
        if self.active_db_target == "local":
            db_password = os.environ.get("DB_PASSWORD") or os.environ.get("LOCAL_DB_PASSWORD") or os.environ.get("PGPASSWORD")
            if not db_password:
                db_password, ok = QInputDialog.getText(
                    self,
                    "Local DB Password",
                    "Enter local PostgreSQL password:",
                ...
                if ok and db_password:
                    ...
                    QMessageBox.warning(self, "Login Error", "Local DB password is required for connection.")
                    return
            # Refresh login manager config after prompt
        ...
        try:
            ...
            err_text = str(e)
            if "password authentication failed" in err_text or "no password supplied" in err_text:
                # Prompt for DB user + password and retry once
                db_user, ok_user = QInputDialog.getText(
                    self,
                    "Local DB User",
                    "Enter local PostgreSQL user:",
                    QLineEdit.EchoMode.Normal,
                    os.environ.get("DB_USER", os.environ.get("LOCAL_DB_USER", "postgres"))
                )
                if not ok_user or not db_user:
                    QMessageBox.warning(self, 'Login Error', 'Database user is required for connection.')
                    return

                db_password, ok_pass = QInputDialog.getText(
                    self,
                    "Local DB Password",
                    f"Enter password for '{db_user}':",
                    QLineEdit.EchoMode.Password
                )
                if not ok_pass or not db_password:
                    QMessageBox.warning(self, 'Login Error', 'Database password is required for connection.')
                    return

                os.environ["DB_USER"] = db_user
                os.environ["LOCAL_DB_USER"] = db_user
                os.environ["DB_PASSWORD"] = db_password
                os.environ["LOCAL_DB_PASSWORD"] = db_password
                self.login_manager._refresh_db_config()
                ...
                try:
                    ...
                    QMessageBox.warning(self, 'Login Error', f'Database auth failed: {retry_err}')
                    self.password_input.clear()
                    self.password_input.setFocus()
                    return

            QMessageBox.warning(self, 'Login Error', f'An unexpected error occurred: {err_text}')
            self.password_input.clear()
            self.password_input.setFocus()
        finally:
