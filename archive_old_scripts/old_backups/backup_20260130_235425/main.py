...
class CharterFormWidget(QWidget):
    ...
    def _open_routing_charges_dialog(self):
        ...
        try:
            if not self.charter_id:
                QMessageBox.information(self, "Routing & Charges", "Save the charter first, then open Routing & Charges.")
                return

            reserve_number = self._fetch_reserve_number(self.charter_id)
            if not reserve_number:
                QMessageBox.warning(self, "Routing & Charges", "Could not find reserve number for this charter.")
                return
    ...
    def create_charter_details_section(self, lock_btn=None, cancel_btn=None, close_btn=None) -> QGroupBox:
        ...
        rate_pricing_layout.addSpacing(10)
        rate_pricing_layout.addWidget(QLabel("Flat Rate:"))
        ...
        routing_shortcut_layout.addWidget(QLabel("Manage routing and charges in the Charter Details window."))
    ...
    def _on_requested_vehicle_type_changed(self):
        ...
        try:
            ...
            # Flat Rate = Hourly Package Rate (flat rate × hours = package total)
            if hourly_package > 0:
            ...
            # Split Rate = Same as hourly (split run uses hourly with timing breaks)
            if hourly_rate > 0:
    ...
    def _on_run_type_changed(self):
        ...
        try:
            ...
            # Load default charges for this run type
            cur = self.db.get_cursor()
            cur.execute("""
                SELECT charge_description, charge_type, amount, calc_type, value, is_taxable, sequence
                FROM run_type_default_charges
                WHERE run_type_id = %s
                ORDER BY sequence
            """, (run_type_id,))
            
            default_charges = cur.fetchall()
            cur.close()
            
            # Add each default charge
            for desc, charge_type, amount, calc_type, value, is_taxable, sequence in default_charges:
                self.add_charge_line(
                    description=desc,
                    charge_type=charge_type or 'other',
                    calc_type=calc_type or 'Fixed',
                    value=float(value) if value else 0.0,
                    is_taxable=is_taxable if is_taxable is not None else True,
                    auto_added=True  # Mark as auto-added for easy removal
                )
                print(f"✅ Auto-added charge: {desc} (${value})")
            ...
            if vehicle_type:
                pricing = self._load_pricing_defaults(vehicle_type)
                if "airport pickup - calgary" in run_type_name:
                ...
                if "airport pickup - edmonton" in run_type_name:
    ...
    def open_run_type_editor(self):
        ...
        from PyQt6.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QTableWidget, QTableWidgetItem, QPushButton, QMessageBox
        ...
        try:
            ...
            for r_idx, row in enumerate(rows):
                ...
                active_item.setCheckState(Qt.CheckState.Checked if active_val else Qt.CheckState.Unchecked)
                ...
                try:
                    ...
                    if has_id:
                        ...
                        for row in range(table.rowCount()):
                            ...
                            is_active = True if not has_active else (active_item.checkState() == Qt.CheckState.Checked)
                            order_item = table.item(row, 2)
                            display_order = int(order_item.text() or (row + 1)) if has_order else None
    ...
    def add_charge_line(self, description: str = "New Charge", calc_type: str = "Fixed", value: float = 0.0, 
                       charge_type: str = "other", is_taxable: bool = True, auto_added: bool = False):
...
def set_active_db(target):
    ...
    if ACTIVE_DB_TARGET == "local":
        ...
        os.environ["DB_NAME"] = os.environ.get("DB_NAME", os.environ.get("LOCAL_DB_NAME", "almsdata"))
        os.environ["DB_USER"] = os.environ.get("DB_USER", os.environ.get("LOCAL_DB_USER", "postgres"))
        os.environ["DB_PASSWORD"] = os.environ.get("DB_PASSWORD", os.environ.get("LOCAL_DB_PASSWORD", ""))
