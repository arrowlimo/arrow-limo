...
class DispatchManagementWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.bookings_table.setHorizontalHeaderLabels([
            "Reserve #", "Date", "Client", "Run Type", "Vehicle Type", "Driver",
            "Status", "Passengers", "Pickup", "Dropoff", "Beverage", "Driver Notes"
        ])
        ...
        try:
            ...
            cur.execute("""
                SELECT 
                    ...
                    c.reserve_number, 
                    c.charter_date::date,
                    COALESCE(c.client_display_name, 'Unknown') as client_name,
                    COALESCE(c.charter_data->>'run_type', '') as run_type,
                    c.vehicle_type_requested, 
                    COALESCE(c.driver_name, 'Unassigned') as driver,
                    COALESCE(c.payment_status, c.status, 'Pending') as status,
                    COALESCE(c.passenger_count, c.passenger_load, 0) as passengers,
                    c.pickup_address, 
                    c.dropoff_address,
                    EXISTS (SELECT 1 FROM charter_beverages cb WHERE cb.charter_id = c.charter_id) as has_beverages,
                    COALESCE(c.notes, c.vehicle_notes, '') as driver_notes
                FROM charters c
                ORDER BY c.charter_date DESC LIMIT 500
            """)
    ...
    def display_bookings(self, bookings):
        ...
        for row_idx, booking in enumerate(bookings):
            cells = [
                ...
                str(booking[4] or ""),  # run_type
                str(booking[5] or ""),  # vehicle_type
                str(booking[6] or ""),  # driver_name
                str(booking[7] or "Pending"),  # status
                str(booking[8] or ""),  # passengers
                str(booking[9] or ""),  # pickup_address
                str(booking[10] or ""),  # dropoff_address
                "Yes" if booking[11] else "",  # beverage flag
                str(booking[12] or ""),  # driver notes
            ]
        ...
        for booking in self.bookings_data:
            ...
            if search_text:
                if not any([
                    ...
                    search_text in (booking[4] or "").lower(),  # run type
                    search_text in (booking[5] or "").lower(),  # vehicle
                    search_text in (booking[6] or "").lower(),  # driver
                    search_text in (booking[9] or "").lower(),  # pickup
                    search_text in (booking[10] or "").lower(),  # dropoff
                    search_text in (booking[12] or "").lower(),  # driver notes
                ]):
                    continue
