...
# UI Style Constants
STYLE_ERROR_LABEL = "color: red; font-weight: bold;"
STYLE_SUCCESS_LABEL = "color: green; font-weight: bold;"
GL_CODE_PLACEHOLDER = "-- Select GL Code --"
...
class SplitReceiptManagerDialog(QDialog):
    ...
    def _load_receipt(self) -> Optional[Dict]:
    ...
    def _build_splits_tab(self) -> QWidget:
        ...
        self.splits_validation_label.setStyleSheet(STYLE_ERROR_LABEL)
    ...
    def _build_banking_tab(self) -> QWidget:
        ...
        self.banking_validation_label.setStyleSheet(STYLE_ERROR_LABEL)
    ...
    def _build_cashbox_tab(self) -> QWidget:
        ...
        self.cashbox_validation_label.setStyleSheet(STYLE_ERROR_LABEL)
    ...
    def _load_splits(self):
        ...
        try:
            ...
            for r, (split_id, order, gl, amt, method, notes) in enumerate(rows):
                self.splits_table.setItem(r, 0, QTableWidgetItem(gl or ""))
                self.splits_table.setItem(r, 1, QTableWidgetItem(f"{amt:.2f}"))
                self.splits_table.setItem(r, 2, QTableWidgetItem(method or ""))
                self.splits_table.setItem(r, 3, QTableWidgetItem(notes or ""))

                del_btn = QPushButton("🗑")
                del_btn.clicked.connect(lambda checked, rid=split_id: self._delete_split(rid))
                self.splits_table.setCellWidget(r, 4, del_btn)
    ...
    def _validate_splits(self):
        ...
        for r in range(self.splits_table.rowCount()):
            ...
            if amt_item:
                try:
                    ...
                    # Invalid amount text - skip this row
                    pass
        ...
        if variance < 0.01:
            ...
            self.splits_validation_label.setStyleSheet(STYLE_SUCCESS_LABEL)
        else:
    ...
    def _extract_split_row_data(self, row_index: int) -> Tuple[str, float, str, str]:
        """Extract GL code, amount, payment method, and category from a table row."""
        # Column 0 is GL Code (ComboBox)
        gl_widget = self.splits_table.cellWidget(row_index, 0)
        ...
        # Column 1 is Amount
        amt_text = self.splits_table.item(row_index, 1).text().strip()
        amt = float(amt_text) if amt_text else 0.0
        
        # Column 2 is Payment Method (ComboBox)
        method_widget = self.splits_table.cellWidget(row_index, 2)
        ...
        category = self.splits_table.item(row_index, 3).text().strip() or ""
        
        return gl, amt, payment_method, category
    
    def _create_child_receipt(self, cursor, gl_code: str, amount: float, 
                             payment_method: str, category: str, 
                             split_group_id: int, original_total: float) -> int:
        """Create a child receipt and return its ID."""
        cursor.execute("""
        ...
        # Create tracking entry in receipt_splits
        cursor.execute("""
            INSERT INTO receipt_splits 
            (parent_id, child_id, child_amount, child_category)
            VALUES (%s, %s, %s, %s)
        """, (self.receipt_id, child_id, amount, category))
        
        return child_id
    
    def _cleanup_original_receipt(self, cursor):
        """Delete the original parent receipt and its split tracking entries."""
        cursor.execute("""
    ...
    def _save_all_splits(self):
        ...
        try:
            ...
            for r in range(self.splits_table.rowCount()):
                gl, amt, payment_method, category = self._extract_split_row_data(r)
                
                if gl and amt > 0 and gl != GL_CODE_PLACEHOLDER:
                    try:
                        child_id = self._create_child_receipt(
                            cur, gl, amt, payment_method, category, 
                            split_group_id, original_total
                        )
            ...
            # Clean up original receipt
            self._cleanup_original_receipt(cur)
            self.conn.commit()
            cur.close()
            
            QMessageBox.information(
                self, "Success", 
                f"✅ Receipt #{self.receipt_id} split into {len(child_ids)} linked receipts!\n\n"
