...
class ReceiptSearchMatchWidget(QWidget):
    ...
    def _auto_resize_table_height(self):
        """Auto-expand table height to fit content (up to max 250px for non-fullscreen)."""
        self.results_table.resizeRowsToContents()
        total_height = self.results_table.horizontalHeader().height()
        for row in range(self.results_table.rowCount()):
            total_height += self.results_table.rowHeight(row)
        
        # Cap at 250px maximum (reduced from 600px for non-fullscreen mode)
        target_height = min(total_height + 4, 250)  # +4 for scrollbar padding
    ...
    def _link_selected_to_banking(self):
        ...
        try:
            ...
            # FIX: After linking, explicitly re-enable selection changes by forcing form refresh
            # This allows user to select a different invoice immediately after linking
            self.results_table.itemSelectionChanged.emit()
            
            QMessageBox.information(self, "Linked", f"Linked receipt #{receipt_id} to banking txn #{transaction_id}.")
            
            # FIX: After message, force repopulation to show updates if receipt still selected
