"""
PDF Form Filler - Payroll & Accounting Functions (FIXED VERSION)
...
FIXES APPLIED:
- [FIXED] Invoice PDF: Safe Decimal conversion for unit_price (was crashing on None)
- [FIXED] Expense Report: Safe None handling for vendor_name and description strings
- [FIXED] All currency fields: Proper Decimal conversion with fallback to 0
"""
...
class PayrollAccountingPDFFiller:
        ...
        try:
            ...
            self.cur.execute("""
                SELECT 
                    ...
                    SUM(COALESCE(t4_box_14, 0)) as box_14,
                    SUM(COALESCE(t4_box_16, 0)) as box_16,
                    SUM(COALESCE(t4_box_18, 0)) as box_18,
                    SUM(COALESCE(t4_box_22, 0)) as box_22,
                    SUM(COALESCE(t4_box_24, 0)) as box_24,
                    SUM(COALESCE(t4_box_26, 0)) as box_26,
                    SUM(COALESCE(t4_box_44, 0)) as box_44,
                    SUM(COALESCE(t4_box_46, 0)) as box_46,
                    SUM(COALESCE(t4_box_52, 0)) as box_52
                FROM driver_payroll
            ...
            # Employee info
            emp_data = [
                ["Employee Name:", employee['full_name'], "SIN:", employee['t4_sin'] or ''],
            ...
            box_data = [
                ...
                ["14", "Employment Income", f"${Decimal(str(t4_data['box_14'] or 0)):,.2f}"],
                ["16", "Employee CPP Contributions", f"${Decimal(str(t4_data['box_16'] or 0)):,.2f}"],
                ["18", "Employee EI Premiums", f"${Decimal(str(t4_data['box_18'] or 0)):,.2f}"],
                ["22", "Income Tax Deducted", f"${Decimal(str(t4_data['box_22'] or 0)):,.2f}"],
                ["24", "EI Insurable Earnings", f"${Decimal(str(t4_data['box_24'] or 0)):,.2f}"],
                ["26", "CPP Pensionable Earnings", f"${Decimal(str(t4_data['box_26'] or 0)):,.2f}"],
                ["44", "Employment Commissions", f"${Decimal(str(t4_data['box_44'] or 0)):,.2f}"],
                ["46", "Deferred Salary/Leave of Absence", f"${Decimal(str(t4_data['box_46'] or 0)):,.2f}"],
                ["52", "Other Income", f"${Decimal(str(t4_data['box_52'] or 0)):,.2f}"],
            ]
    ...
    def generate_invoice_pdf(self, invoice_id: int, output_path: str = None) -> Optional[str]:
        """Generate invoice PDF (FIXED: Safe Decimal conversion for unit_price)"""
        ...
        try:
            ...
            # Invoice header
            amount_val = Decimal(str(invoice['amount'] or 0))
            header_data = [
                ["Invoice #:", str(invoice['invoice_number']), "Date:", str(invoice['invoice_date'])],
                ["Status:", str(invoice['status']).upper(), "Amount:", f"${amount_val:,.2f}"],
            ]
            ...
            for item in items:
                ...
                # FIXED: Safe Decimal conversion for unit_price (was crashing on None)
    ...
    def generate_expense_report(self, start_date: date, end_date: date, output_path: str = None) -> Optional[str]:
        """Generate expense report PDF (FIXED: Safe None handling for strings)"""
        ...
        try:
            ...
            self.cur.execute("""
                SELECT receipt_id, receipt_date, vendor_name, expense_account, description,
                       gross_amount, payment_method, validation_reason
                FROM receipts
            ...
            if not receipts:
                print(f"No receipts found between {start_date} and {end_date}")
                return None
            ...
            period_table = Table(period_data, colWidths=[1.5*inch, 2*inch])
            ...
            # Expense items
            exp_data = [["Date", "Vendor", "Category", "Description", "Amount"]]
            total_amount = Decimal('0')
            
            for receipt in receipts:
                # FIXED: Safe None handling for strings (was crashing with slicing on None)
                vendor = receipt['vendor_name'][:20] if receipt['vendor_name'] else ''
                category = receipt['expense_account'][:15] if receipt['expense_account'] else 'Other'
                description = receipt['description'][:30] if receipt['description'] else ''
                
                # Safe Decimal conversion
                amount = Decimal(str(receipt['gross_amount'] or 0))
                
                exp_data.append([
                    str(receipt['receipt_date']),
                    vendor,
                    (receipt['expense_account'] or 'Other')[:15] if receipt['expense_account'] else 'Other',
                    description,
                    f"${amount:,.2f}",
                ])
                total_amount += amount
            
            exp_data.append(["", "", "", "TOTAL:", f"${total_amount:,.2f}"])
            
            exp_table = Table(exp_data, colWidths=[0.8*inch, 1.2*inch, 1*inch, 2*inch, 1*inch])
    ...
    def generate_vendor_statement(self, vendor_name: str, output_path: str = None) -> Optional[str]:
        ...
        try:
            ...
            self.cur.execute("""
                SELECT payable_id, invoice_date, amount, payment_date, status
                FROM payables
                WHERE vendor_name = %s
            ...
            elements.append(Paragraph("VENDOR STATEMENT", title_style))
            elements.append(Spacer(1, 0.15*inch))
            
            # Vendor info
            vendor_data = [
                ["Vendor:", vendor_name, "Statement Date:", str(datetime.now().date())],
            ]
            vendor_table = Table(vendor_data, colWidths=[1*inch, 2*inch, 1.2*inch, 1.5*inch])
            ...
            # Invoice items
            inv_data = [["Invoice Date", "Amount", "Paid Date", "Status", "Balance"]]
            total_due = Decimal('0')
            ...
            for invoice in invoices:
                ...
                inv_data.append([
                    ...
                    f"${amount:,.2f}",
                    str(invoice['payment_date'] or ''),
                    status,
                    f"${balance:,.2f}",
                ])
            
            inv_data.append(["", "", "", "Total Due:", f"${total_due:,.2f}"])
            
            inv_table = Table(inv_data, colWidths=[1*inch, 1*inch, 1*inch, 1*inch, 1*inch])
...
if __name__ == "__main__":
    ...
    print("[1] Testing pay stub generation...")
    try:
        result = filler.generate_paystub(1, 2025, 12)
        print(f"     Result: {result}\n" if result else "     No data available\n")
    except Exception as e:
        print(f"     Error: {e}\n")
    
    print("[2] Testing T4 slip generation...")
    try:
        result = filler.generate_t4_slip(1, 2025)
        print(f"     Result: {result}\n" if result else "     No data available\n")
    except Exception as e:
        print(f"     Error: {e}\n")
    
    print("[3] Testing invoice PDF generation...")
    try:
        result = filler.generate_invoice_pdf(1)
        print(f"     Result: {result}\n" if result else "     No data available\n")
    except Exception as e:
        print(f"     Error: {e}\n")
    
    print("[4] Testing expense report generation...")
    try:
        from datetime import date, timedelta
        end = date.today()
        start = end - timedelta(days=30)
        result = filler.generate_expense_report(start, end)
        print(f"     Result: {result}\n" if result else "     No data available\n")
    except Exception as e:
        print(f"     Error: {e}\n")
    
    print("[5] Testing vendor statement generation...")
    try:
        result = filler.generate_vendor_statement("Arrow Limousine")
        print(f"     Result: {result}\n" if result else "     No data available\n")
    except Exception as e:
        print(f"     Error: {e}\n")
