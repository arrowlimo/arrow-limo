#!/usr/bin/env python3
"""Check for Gin Smash, Cottage Springs, No Way Ice Tea, Corona, Keystone."""
import psycopg2
import os
...
def check_products():
    ...
    searches = [
        ("Gin Smash", "%gin smash%"),
        ("Cottage Springs", "%cottage%"),
        ("No Way Ice Tea", "%no way%"),
        ("Corona", "%corona%"),
        ("Keystone", "%keystone%"),
    ]
    ...
    for search_name, pattern in searches:
        ...
        rows = cur.fetchall()
        if rows:
            print(f"✅ Found {len(rows)} {search_name} product(s):")
