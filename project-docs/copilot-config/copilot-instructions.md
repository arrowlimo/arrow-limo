...
## 🚨🚨🚨 CRITICAL SECURITY WARNINGS - READ FIRST 🚨🚨🚨

### ⛔ ABSOLUTE RULES - NO EXCEPTIONS - LIFE/BUSINESS AT RISK ⛔

**1. NEVER COMMIT PASSWORDS OR CREDENTIALS TO GIT - EVER**
- ❌ NO hardcoded passwords in ANY file that goes to GitHub
- ❌ NO database credentials in scripts, configs, or test files
- ❌ NO API keys, tokens, or connection strings
- ✅ ALWAYS use environment variables: `os.getenv("DB_PASSWORD")`
- ✅ ALWAYS add credential-containing files to `.gitignore`
- ⚠️ **Violations cause security breaches, stress, and business risk**

**2. BEFORE ANY `git commit` OR `git push`:**
- Run: `git diff --cached` to review what you're about to commit
- Search for: passwords, "npg_", "***REDACTED***", connection strings
- If found: STOP, remove from files, use environment variables
- Add files with credentials to `.gitignore` immediately

**3. IF YOU ACCIDENTALLY COMMIT CREDENTIALS:**
- STOP all work immediately
- Add file to `.gitignore`
- Run: `git rm --cached <filename>`
- Use BFG Repo Cleaner to scrub git history
- Force push cleaned history
- Inform user to rotate credentials

**4. WEB APP REPOSITORY - KEEP CLEAN:**
- Only commit: `modern_backend/`, `frontend/`, deployment files
- Do NOT commit: `desktop_app/`, `scripts/`, `data/`, `docs/`, test files
- Keep repository lean for web deployment only
