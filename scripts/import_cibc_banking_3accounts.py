#!/usr/bin/env python3
"""
CIBC Banking Import - Import all 3 accounts (Nov 2025 - Jan 2026)
- Handles 3 CIBC accounts: 0228362 (checking), 3648117 (business deposit), 8314462 (vehicle loans)
...
# CIBC Account mappings
CIBC_ACCOUNTS = {
    "0228362": {
        "name": "CIBC Checking Account",
        "path": r"l:\limo\CIBC UPLOADS\0228362 (CIBC checking account)\cibc8362.csv",
        "bank_id": 1,  # mapped_bank_account_id
        "type": "checking"
    },
    "3648117": {
        "name": "CIBC Business Deposit Account",
        "path": r"l:\limo\CIBC UPLOADS\3648117 (CIBC Business Deposit account, alias for 0534\cibc8117.csv",
        "bank_id": 1,  # Same bank, checking account
        "type": "savings"
    },
    "8314462": {
        "name": "CIBC Vehicle Loans Account",
        "path": r"l:\limo\CIBC UPLOADS\8314462 (CIBC vehicle loans)\cibc4462.csv",
        "bank_id": 1,  # Same bank, different account
        "type": "loan"
    }
}
...
def import_cibc_account(cur, account_id: str, account_info: dict) -> Tuple[int, int]:
    """
    Import transactions from one CIBC account.
    Returns: (imported_count, skipped_count)
    """
    csv_path = account_info["path"]
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row_num, row in enumerate(reader, 1):
                ...
                cur.execute("""
                    ...
                    LIMIT 1
                """, (date_str, description, float(amount), float(amount)))
                ...
                if WRITE and not DRY_RUN:
                    try:
                        ...
                        cur.execute("""
                            ...
                            (receipt_date, vendor_name, expense, expense_account, created_from_banking, notes)
                            VALUES (%s, %s, %s, %s, %s, %s)
                        """, (
                            date_str,
                            description[:60],
                    ...
                    imported += 1
                    gl_code = fuzzy_categorize(description)
                    print(f"   OK {date_str} | {description[:40]:40s} | ${amount:8.2f} | {gl_code}")
...
def generate_import_summary(cur):
    ...
    if result:
        count, total = result
        print(f"[OK] Receipts from banking: {count} (${total:.2f})")

if __name__ == "__main__":
    print("CIBC BANKING IMPORT - 3 ACCOUNTS (Nov 2025 - Jan 2026)")
    ...
    with get_db_conn() as conn:
        with conn.cursor() as cur:
            for account_id, account_info in CIBC_ACCOUNTS.items():
                imported, skipped = import_cibc_account(cur, account_id, account_info)
