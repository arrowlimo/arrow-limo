#!/usr/bin/env python3
"""Import CIBC vehicle loan account 8314462 data for Nov-Dec 2025."""
import psycopg2
import csv
...
print("=" * 100)
print("IMPORTING CIBC VEHICLE LOAN ACCOUNT 8314462 - NOV/DEC 2025")
print("=" * 100)
print()

# CSV file
csv_file = r"L:\limo\CIBC UPLOADS\8314462 (CIBC vehicle loans)\2025cibc (2).csv"
...
with open(csv_file, 'r', encoding='utf-8-sig') as f:
    ...
    for row in reader:
        ...
        tx_date = datetime.strptime(row[0].strip(), '%Y-%m-%d').date()
        ...
        hash_input = f"8314462|{tx_date}|{description}|{debit_amt}|{credit_amt}"
...
# Filter to Nov-Dec 2025 only (exclude Jan 2026)
...
print(f"{'Month':10} | {'Count':>6} | {'Debits':>12} | {'Credits':>12}")
print("-" * 50)
for month, count, debits, credits in cur.fetchall():
    debits = debits or 0.0
    credits = credits or 0.0
    print(f"{month:10} | {count:6,} | ${debits:>11,.2f} | ${credits:>11,.2f}")
