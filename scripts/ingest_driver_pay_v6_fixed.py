...
Fixed: unique constraint now uses a generated column hash instead of COALESCE in constraint.
"""
import os
import re
import sys
...
def process_file(cur, path: Path, year_filter=None):
    ...
    if year_filter:
        if not fallback_date:
            print(f"⏭️ {path.name}: No date in filename, skipping with year filter")
            return
            if fallback_date.year not in year_filter:
                print(f"⏭️ {path.name}: {fallback_date.year} not in filter {year_filter}")
                return
