#!/usr/bin/env python3
"""Backup and delete all 2012 CIBC 0228362 banking data (including cheque_register refs)."""
...
print("=" * 100)
print("BACKUP AND DELETE 2012 CIBC 0228362 DATA (with cheque_register cleanup)")
print("=" * 100 + "\n")
...
# Step 3: Backup cheque_register entries
print("\nSTEP 3: Backing up cheque_register entries...")
cur.execute("""
    SELECT cr.* FROM cheque_register cr
    WHERE cr.banking_transaction_id IN (
        SELECT transaction_id FROM banking_transactions
        WHERE account_number = '0228362'
        AND EXTRACT(YEAR FROM transaction_date) = 2012
    )
...
cr_cols = [d[0] for d in cur.description]
cr_rows = cur.fetchall()

backup_data['cheque_register'] = {
    'columns': cr_cols,
    'data': [dict(zip(cr_cols, row)) for row in cr_rows]
}

print(f"  Backed up {len(cr_rows)} cheque_register entries")
...
print("\n" + "=" * 100)
print("STEP 4: Deleting data (user confirmation required)")
print("=" * 100)
print(f"\nAbout to delete:")
print(f"  • {len(cr_rows)} cheque_register entries")
...
if user_confirm == "DELETE":
    ...
    # Delete cheque_register first (FK constraint)
    cur.execute("""
    ...
    deleted_cheques = cur.rowcount
    print(f"✅ Deleted {deleted_cheques} cheque_register entries")
    conn.commit()
    
    # Delete auto-generated receipts
    cur.execute("""
    ...
    if remaining == 0:
        ...
        print("=" * 100)
        print(f"\nDeleted:")
        print(f"  • {deleted_cheques} cheque_register entries")
