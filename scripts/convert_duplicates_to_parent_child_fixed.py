...
38 groups identified with different regional contact info.
"""
...
def convert_duplicates_to_parent_child(dry_run=True):
    ...
    for group_idx, (dup_name, client_ids) in enumerate(duplicates, 1):
        ...
        for cid in client_ids:
            cur.execute("""
                SELECT c.client_id, c.company_name, c.client_name, c.primary_phone, c.email, 
            ...
            # Count charters separately
            cur.execute("SELECT COUNT(*) FROM charters WHERE client_id = %s", (cid,))
            charter_count = cur.fetchone()[0]
            
            if info:
                print(f"  ID {info[0]}: {info[5]}, {info[6]} | {info[3]} | {info[4]} ({charter_count} charters)")
        ...
        if dry_run:
            ...
            cur.execute("""
                SELECT c.client_id, COUNT(ch.charter_id) as charter_count
                FROM clients c
            ...
            primary_id = primary_result[0]
            print(f"  Primary (most charters): ID {primary_id}")
            
            # Create NEW parent with company_name
            cur.execute("""
            ...
            # Insert new parent (copy of primary)
            cur.execute("""
                INSERT INTO clients 
                (company_name, is_company, primary_phone, email, 
            ...
            # Convert all old duplicates to children of new parent
