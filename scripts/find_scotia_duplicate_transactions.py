"""
Find duplicate transactions in Scotia Bank account 903990106011.

Identifies transactions with same date, amount, and similar descriptions
that may be causing the wild balance oscillations observed in the audit.

Usage:
    python find_scotia_duplicate_transactions.py          # Dry-run
    python find_scotia_duplicate_transactions.py --write  # Mark duplicates for review
"""
...
def find_duplicates(cur):
    ...
    - Same debit_amount OR same credit_amount (one must match)
    ...
    # Group by date + amount (debit or credit)
    ...
    for txn in transactions:
        ...
        # Create keys for both debit and credit
        if debit > 0:
            key = (date, 'D', debit)
...
def display_duplicates(duplicates):
    ...
    for i, dup_group in enumerate(duplicates, 1):
        ...
        # Count duplicates (keep 1, rest are duplicates)
...
def mark_duplicates_for_review(cur, duplicates, write=False):
    ...
    Strategy: Keep the FIRST transaction in each group (lowest transaction_id),
    mark others with a comment indicating they are potential duplicates.
    """
    if not write:
    ...
    for dup_group in duplicates:
        ...
        # Sort by transaction_id to keep the first one
        ...
        for txn in txns_sorted[1:]:
            ...
            # Add duplicate marker to description
    ...
    print("These transactions have '[DUPLICATE?]' prefix in their description")
    print("Review manually and delete if confirmed duplicates")


def main():
    parser = argparse.ArgumentParser(description='Find duplicate Scotia Bank transactions')
