#!/usr/bin/env python3
"""
Create banking event receipts for CIBC 3648117.
Apply same patterns as 0228362: ATM, branch withdrawals, fees.
"""
...
def create_banking_events_3648117(dry_run=True):
    conn = get_db_connection()
    cur = conn.cursor()
    
    print("=" * 100)
    print(" " * 20 + "CREATE BANKING EVENT RECEIPTS - CIBC 3648117")
    print("=" * 100)
    print()
    ...
    cur.execute("""
        ...
        WHERE account_number = '3648117'
          ...
          AND description NOT ILIKE '%EFT%'
        ORDER BY transaction_date
    """)
    
    transfer_txns = cur.fetchall()
    print(f"    Found {len(transfer_txns)} transfers (${sum(t[2] for t in transfer_txns):,.2f})")
    print()
    
    total_txns = len(atm_txns) + len(branch_txns) + len(fee_txns) + len(memo_txns) + len(transfer_txns)
    total_amount = sum(t[2] for t in atm_txns + branch_txns + fee_txns + memo_txns + transfer_txns)
    ...
    if not dry_run:
        ...
        for trans_id, date, amount, desc in atm_txns:
            ...
            cur.execute("""
                INSERT INTO receipts (
                    ...
                    'CIBC_3648117_Banking', %s, %s, 'Cash Withdrawal', %s, %s,
        ...
        for trans_id, date, amount, desc in fee_txns:
            ...
            cur.execute("""
                INSERT INTO receipts (
                    ...
                    'CIBC_3648117_Banking', %s, %s, %s, %s, %s,
        ...
        for trans_id, date, amount, desc in transfer_txns:
            source_ref = f"banking_{trans_id}"
            source_hash = hashlib.sha256(f"{trans_id}-{date}-{amount}".encode()).hexdigest()
            ...
            cur.execute("""
                INSERT INTO receipts (
                    ...
                    'CIBC_3648117_Banking', %s, %s, 'Internal Transfer', %s, %s,
                    'Bank Charges', %s, %s, CURRENT_TIMESTAMP,
                    true, 'banking_transaction'
                )
                RETURNING id
            """, (source_ref, date, desc or f"Transfer ${amount:.2f}",
                  amount, trans_id, source_hash))
        ...
        print(f"  {len(transfer_txns)} transfer receipts")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Create banking event receipts for CIBC 3648117')
    parser.add_argument('--write', action='store_true', help='Apply changes (default is dry-run)')
    args = parser.parse_args()
    
    create_banking_events_3648117(dry_run=not args.write)
