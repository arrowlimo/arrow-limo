#!/usr/bin/env python3
"""
Step 1 (Final): Disable triggers, delete receipts for verified years, re-enable triggers.
"""
import psycopg2
...
def main():
    ...
    if banking_refs > 0:
        ...
        print("→ Will temporarily disable banking lock trigger")
    
    print(f"\n{'='*70}")
    print("READY TO PROCEED?")
    print(f"{'='*70}")
    print(f"  1. DISABLE banking lock trigger")
    print(f"  2. NULL out {banking_refs:,} banking_transactions.receipt_id references")
    print(f"  3. DELETE {total_to_delete:,} receipts from 2012-2017")
    print(f"  4. RE-ENABLE banking lock trigger")
    print(f"  5. Remaining: {remaining:,} receipts")
    print(f"{'='*70}\n")
    ...
    if response.strip().upper() == 'DELETE':
        try:
            print("\n🔓 Step 1: Disabling banking lock trigger...")
            
            # Disable the trigger
            cur.execute("""
                ALTER TABLE banking_transactions DISABLE TRIGGER ALL;
            """)
            print("✅ Triggers disabled")
            
            print("\n📝 Step 2: Clearing foreign key references...")
            ...
            print("\n🗑️  Step 3: Deleting receipts for years 2012-2017...")
            ...
            print("\n🔒 Step 4: Re-enabling banking lock trigger...")
            
            # Re-enable the trigger
            cur.execute("""
                ALTER TABLE banking_transactions ENABLE TRIGGER ALL;
            """)
            ...
            print(f"\n❌ ERROR: {e}")
            print("\n🔒 Re-enabling triggers after error...")
