...
def main():
    ...
    if args.write:
        conn.commit()
        print(f"✅ Committed: {inserts} inserts, {updates} updates, {skips} skips")
    else:
        conn.rollback()
        print(f"🔎 Dry-run: {inserts} potential inserts, {updates} potential updates, {skips} skips")
...
- revenue, expense -> determine gross_amount:
    • If expense present (>0), gross_amount = expense (expense receipt)
    • Else if revenue present (>0), gross_amount = revenue (income receipt)
    • Else skip row
