#!/usr/bin/env python
"""
Import 2010 charge summary CSV into general_ledger to backfill missing 2010 GL data.

Source file: l:/limo/recreated_2010_charge_summary.csv
...
Provenance: source_file = 'CHARGES:2010_recreated_csv'
...
CSV_PATH = r"L:/limo/recreated_2010_charge_summary.csv"
...
def already_inserted(cur, date, num, account, credit) -> bool:
    cur.execute(
        ...
        WHERE date = %s AND COALESCE(num,'') = COALESCE(%s,'')
          AND account = %s AND COALESCE(credit,0) = %s AND source_file = 'CHARGES:2010_recreated_csv'
        LIMIT 1
        """,
...
def import_2010():
    ...
    df2010 = df[(df["Reserve Date"].dt.year == 2010)]

    print(f"Rows in 2010: {len(df2010):,}")
    if len(df2010) == 0:
        return 0
    ...
    for idx, row in df2010.iterrows():
        try:
            ...
            if base and base > 0:
                if not already_inserted(cur, date, num, "4000 Limousine Service Income", base):
                    cur.execute(
                        ...
                        VALUES (%s, %s, %s, %s, %s, %s, 'CHARGES:2010_recreated_csv', NOW())
                        """,
...
def main():
    print("IMPORT 2010 CHARGE SUMMARY -> GENERAL LEDGER")
    print("=" * 80)
    total = import_2010()
    print("=" * 80)
