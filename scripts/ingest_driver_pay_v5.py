...
def parse_qbo(path: Path) -> pd.DataFrame:
    # Try XML; otherwise return empty for now (some QBO are SGML)
    try:
            ...
            for page in pdf.pages:
                ...
                for line_no, line in enumerate(text.splitlines(), start=1):
                    ...
                    if amt_m and name_m:
                        ...
                        d = pd.to_datetime(date_m.group(1), errors='coerce') if date_m else pd.NaT
                        rows.append({
                            'txn_date': d,
...
def insert_rows(conn, file_id: int, df: pd.DataFrame, source_file: str) -> int:
    ...
    # Ensure required columns exist (including pay_type)
    ...
    # Replace NA/NaT with None across the DataFrame
    df = df.where(pd.notna(df), None)

    df['file_id'] = file_id
    if 'source_line_no' not in df.columns or df['source_line_no'] is None or pd.isna(df['source_line_no']).all():
