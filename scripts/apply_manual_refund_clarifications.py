...
if charter:
    ...
    cur.execute("""
        ...
        SET charter_id = %s, reserve_number = '011688',
            description = description || ' | Linked to 011688 - balance refund for alcohol order'
        WHERE id = 12
    """)
...
cur.execute("""
    UPDATE charter_refunds 
    SET description = description || ' | REVIEWED: Accidental charge, charged then immediately refunded'
    WHERE id = 1060
""")
...
cur.execute("""
    UPDATE charter_refunds 
    SET description = description || ' | REVIEWED: Accidental charge, no match needed'
    WHERE id = 1054
""")
...
cur.execute("""
    UPDATE charter_refunds 
    SET description = description || ' | REVIEWED: Run cancelled, no charter exists to link'
    WHERE id = 1055
""")
...
if candidates:
    ...
    cur.execute("""
        UPDATE charter_refunds 
        SET description = description || ' | REVIEWED: Duplicate charge refund - searching for deposit/charter'
        WHERE id = 1052
    """)
...
print("\n" + "=" * 80)
print("CURRENT REFUND LINKAGE STATUS")
print("=" * 80)

cur.execute("""
    SELECT 
        COUNT(*) FILTER (WHERE reserve_number IS NOT NULL) as linked,
        COUNT(*) FILTER (WHERE reserve_number IS NULL) as unlinked,
        COUNT(*) as total,
        SUM(amount) FILTER (WHERE reserve_number IS NOT NULL) as linked_amount,
        SUM(amount) FILTER (WHERE reserve_number IS NULL) as unlinked_amount,
...
if unlinked_refunds:
    ...
    print(f"{'ID':<8} {'Date':<12} {'Amount':<12} {'Status':<50}")
    print("-" * 90)
    for row in unlinked_refunds:
        rid, date, amount, desc = row
        # Truncate description
        status = 'REVIEWED' if 'REVIEWED:' in (desc or '') else 'Needs review'
        desc_short = (desc or '')[:47] + '...' if desc and len(desc) > 50 else (desc or '')
        print(f"{rid:<8} {str(date):<12} ${float(amount):<11,.2f} {desc_short:<50}")

print("\n" + "=" * 80)
print("COMMIT CHANGES? (DRY RUN)")
print("=" * 80)
print("\nTo apply these changes, run with --write flag:")
print("  python apply_manual_refund_clarifications.py --write")
print("\nChanges to apply:")
print("  ✓ Link refund #12 to Reserve 011688")
print("  ✓ Mark refunds #1060, #1054, #1055 as reviewed/non-linkable")
print("  ✓ Mark refund #1052 for deposit search")
