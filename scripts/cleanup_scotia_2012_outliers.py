#!/usr/bin/env python3
"""
Clean up erroneous Scotia 2012 banking_transactions rows caused by OCR balance mis-parsing.

Criteria:
  account_number = '903990106011'
  transaction_date BETWEEN '2012-01-01' AND '2012-12-31'
  AND (debit_amount > 50000 OR credit_amount > 50000)
...
def main():
    ...
    ap.add_argument('--threshold', type=float, default=50000.0, help='Amount threshold to consider outliers')
    args = ap.parse_args()

    table = 'banking_transactions'
    cond = (
        f"account_number = '{args.account}' AND "
        f"transaction_date >= '{args.from_date}' AND transaction_date <= '{args.to_date}' AND "
        f"(COALESCE(debit_amount,0) > {args.threshold} OR COALESCE(credit_amount,0) > {args.threshold})"
    )
