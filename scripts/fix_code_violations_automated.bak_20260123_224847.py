...
VIOLATIONS = {
    'charter_id_business_logic': {
        'patterns': [
            ...
            # Pattern: p.charter_id IS NULL (unmatched payments)
...
class ViolationFixer:
    ...
    def fix_charter_id_business_logic(self, content, file_path):
        ...
        # Pattern 1: WHERE payments.charter_id IS NULL (unmatched payments)
        # This is checking if payment is not linked to charter - use reserve_number instead
        pattern = r'WHERE\s+(\w+\.)?charter_id\s+IS\s+NULL'
        if re.search(pattern, content):
            # Check context - is this in a WHERE clause for business logic?
            # Safe fix: This pattern indicates unmatched records (business logic)
            # Replace with checking if reserve_number is NULL instead
            content = re.sub(
                r'WHERE\s+payments\.charter_id\s+IS\s+NULL',
            ...
            if content != original:
                fixes.append('Fixed: charter_id IS NULL → reserve_number IS NULL (unmatched records check)')
        
        # Pattern 2: WHERE charter_id = %s in payment matching context
        # Check if this is in a payment context
        if 'WHERE' in content and 'charter_id' in content and 'payments' in content:
            # This might be a legitimate relationship check - be conservative
            # Flag for review rather than auto-fix
            pass
