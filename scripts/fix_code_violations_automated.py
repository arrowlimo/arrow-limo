...
VIOLATIONS = {
    'charter_id_business_logic': {
        'patterns': [
            # Pattern: WHERE reserve_number IS NULL/NOT NULL (business logic check)
...
class ViolationFixer:
    ...
    def fix_charter_id_business_logic(self, content, file_path):
        ...
        # Pattern 1: WHERE reserve_number IS NULL (most common - unmatched records)
        # Must do unaliased first, then aliased
        
        # Unaliased: WHERE reserve_number IS NULL
        if re.search(r'WHERE\s+charter_id\s+IS\s+NULL', content, re.IGNORECASE):
            ...
            fixes.append('Fixed: WHERE reserve_number IS NULL → WHERE reserve_number IS NULL')
        
        # Aliased: WHERE p.reserve_number IS NULL  
        if re.search(r'WHERE\s+\w+\.charter_id\s+IS\s+NULL', content, re.IGNORECASE):
            ...
            fixes.append('Fixed: aliased WHERE x.reserve_number IS NULL → WHERE x.reserve_number IS NULL')
        
        # Pattern 2: WHERE reserve_number IS NOT NULL
        if re.search(r'WHERE\s+charter_id\s+IS\s+NOT\s+NULL', content, re.IGNORECASE):
            ...
            fixes.append('Fixed: WHERE reserve_number IS NOT NULL → WHERE reserve_number IS NOT NULL')
        
        # Aliased: WHERE p.reserve_number IS NOT NULL
        if re.search(r'WHERE\s+\w+\.charter_id\s+IS\s+NOT\s+NULL', content, re.IGNORECASE):
            ...
            fixes.append('Fixed: WHERE x.reserve_number IS NOT NULL → WHERE x.reserve_number IS NOT NULL')
