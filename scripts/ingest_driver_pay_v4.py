...
def insert_rows(conn, file_id: int, df: pd.DataFrame, source_file: str) -> int:
    ...
    required = ['driver_id','gross_amount','expense_amount','net_amount','currency','account','category','vendor','source_sheet','check_no','memo','driver_name','amount','txn_date','source_row_id']
    for c in required:
    ...
    # Convert NaT/NaN to None for DB
    df['txn_date'] = df['txn_date'].apply(_none_if_nan)
    for c in ['amount','gross_amount','expense_amount','net_amount']:
        df[c] = df[c].apply(_none_if_nan)

    df['file_id'] = file_id
    if 'source_line_no' not in df.columns or df['source_line_no'].isna().all():
...
def process_file(conn, path: Path) -> None:
    ...
    try:
        ...
        # Rollback and record error so we can continue with other files
        conn.rollback()
        try:
