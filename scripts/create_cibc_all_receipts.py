#!/usr/bin/env python3
"""
Create receipts for ALL CIBC Bank transactions across all years.
"""
...
def extract_vendor_name(description):
    ...
    desc = re.sub(r'^CK\s+\d+\s*', '', desc, flags=re.IGNORECASE)
    ...
    if not desc or desc.upper() in ['CHQ', 'CHEQUE', 'CK', '']:
...
def determine_category(description):
    ...
    # Bank Fees
    if any(keyword in desc for keyword in ['SERVICE FEE', 'MONTHLY FEE', 'NSF FEE', 'NSF CHECK', 'MERCHANT FEE', 'MERCHANT SERVICES']):
        return "Bank Fees"
    
    # Income - Card Payments (Global Payments merchant services)
    if any(keyword in desc for keyword in ['GLOBAL', 'GBL', 'DEPOSIT', 'VCARD', 'MCARD', 'CREDIT MEMO']):
    ...
    if 'CHQ' in desc or 'CHEQUE' in desc or 'CK' in desc:
        return "Unknown"
...
def main():
    ...
    try:
        # First, check coverage by year
        print("=" * 80)
        print("CIBC BANK RECEIPT COVERAGE BY YEAR (ALL YEARS)")
        print("=" * 80)
        ...
        cur.execute("""
            ...
            WHERE b.bank_id = 1
            GROUP BY EXTRACT(YEAR FROM b.transaction_date)
            ORDER BY year
        """)
        ...
        for row in years_data:
            year, total, with_receipts, without = row
            rate = (with_receipts / total * 100) if total > 0 else 0
            print(f"{int(year)}: {total:5} total | {with_receipts:5} receipts | {without:5} MISSING | {rate:5.1f}% matched")
            total_missing += without
        
        print()
        print(f"Total missing across all years: {total_missing}")
        print()
        ...
        if not transactions:
            print("✅ All CIBC Bank transactions already have receipts!")
            return
        ...
        print()
        print(f"Verification: {remaining} CIBC transactions still need receipts")
