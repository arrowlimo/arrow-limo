#!/usr/bin/env python3
"""
Delete ONLY debit rows for Scotia account 903990106011 in 2012. Keep credits (deposits).
...
def main():
    ...
    cur.execute(f"SELECT COUNT(*), COALESCE(SUM(debit_amount),0) FROM {table} WHERE {cond}")
    count, sum_debits = cur.fetchone()
    print(f"Target debit rows: {count} | Debits sum: {sum_debits}")
    ...
    print(f"Deleted {deleted} debit rows from {table} (backup: {backup})")
