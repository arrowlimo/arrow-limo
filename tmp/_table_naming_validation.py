...
for root in roots:
    for f in root.rglob('*.py'):
        ...
        rel = f.relative_to(Path('l:/limo')).as_posix()
        for m in pat.finditer(src):
            t = m.group(1).lower()
            if len(t) <= 3:
                continue
            if t.startswith(('pg_', 'information_', 'public')):
                continue
            refs.setdefault(t, set()).add(rel)

missing = {t: sorted(v) for t, v in refs.items() if t not in real_tables}

print(f'Real tables in DB: {len(real_tables)}')
print(f'Distinct table names referenced in code: {len(refs)}')
print(f'Referenced but NOT in DB: {len(missing)}')
for t in sorted(missing):
    print(f'{t}: {missing[t][0]}')
