-- TIMEOUT FIX - CLEAN VERSION (No Unicode/Emojis)
-- Copy and paste this entire script into pgAdmin Query Tool

-- Step 1: Increase timeouts (gives you 2 hours to work)
SET idle_in_transaction_session_timeout = '2h';
SET statement_timeout = '15min';

SELECT 'SUCCESS: You now have 2 HOURS to work on receipts without timeout!' as status;

-- Step 2: Add missing indexes (makes receipts load instantly)
...
SELECT 
    CASE 
        WHEN COUNT(*) >= 6 THEN 'SUCCESS! All 6 indexes created. Receipts will now load INSTANTLY!'
        ELSE 'WARNING: Only ' || COUNT(*) || ' of 6 indexes exist. Check for errors above.'
    END as final_status
FROM pg_indexes
...
-- DONE! Now restart your desktop application and test opening a receipt.
-- It should load in under 1 second and never timeout while you're working!
