...
def list_bookings():
    sql = """
        SELECT
            ...
            c.passenger_count,
            c.vehicle_id,
            COALESCE(NULLIF(v.vehicle_number, ''), NULLIF(c.vehicle, ''),
                '') AS vehicle,
            COALESCE(v.make || ' ' || v.model, '') AS vehicle_description,
            COALESCE(NULLIF(v.vehicle_type, ''), NULLIF(c.vehicle, ''),
                '') AS vehicle_type_requested,
            COALESCE(NULLIF(e.first_name || ' ' || e.last_name, ''),
                NULLIF(c.driver, ''), '') AS driver_name,
            ...
            (COALESCE(c.total_amount_due, 0) - COALESCE(p.total_paid,
            0)) AS balance,
            CASE
        ...
        items.append(
            {
                ...
                "vehicle_type_requested": rec.get(
...
def create_booking(request: Request, payload: dict[str, Any] | None = None):  # noqa: C901
    ...
    # Fixed: DB column is driver_notes
    ...
    with cursor() as cur:
        if not client_id:
            ...
            if row:
                ...
                cur.execute(
                    """
                    INSERT INTO clients (account_number, client_name, phone,
                    email, billing_address, city, province, zip_code)
        ...
        # Generate reserve_number via sequence if available; else fallback to
        # max+1
        reserve_number = None
        try:
        ...
        if isinstance(itinerary, list) and len(itinerary) > 0:
            ...
            for seq, stop in enumerate(itinerary, start=1):
                ...
                # For route segments, each stop creates a row with that
                # location as pickup
        ...
        if not is_gst_exempt and total_amount_due_float > 0:
            ...
            if gst_amount > 0:
                cur.execute(
                    """
                    INSERT INTO charges (reserve_number, charge_type, amount,
                    description)
                    VALUES (%s, %s, %s, %s)
                    """,
                    (
