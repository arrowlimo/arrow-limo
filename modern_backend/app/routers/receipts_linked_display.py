    ...
    try:
        ...
        cur.execute(
            ...
            SELECT DISTINCT r.receipt_id, r.receipt_date, r.vendor_name, r.canonical_vendor,
                   ...
                   v.vehicle_number, r.is_paper_verified, COALESCE(r.is_split_receipt, FALSE)
            FROM receipts r
            ...
            WHERE 
                ...
                OR (r.receipt_id = %s AND (
                    ...
                    OR EXISTS (SELECT 1 FROM receipts r2 WHERE r2.split_group_id = r.split_group_id AND r.split_group_id IS NOT NULL)
            ...
            """,
            (banking_id, parent_id, split_group_id, split_key, split_key, receipt_id),
        )
    ...
    try:
        cur.execute(
            ...
            SELECT r.receipt_id, r.receipt_date, r.vendor_name, r.canonical_vendor,
                   r.gross_amount, r.gst_amount, r.description, r.vehicle_id, r.fuel_amount,
                 r.banking_transaction_id, r.gl_account_code, r.parent_receipt_id,
                 r.split_group_id, r.split_key,
                 COALESCE(r.owner_personal_amount, 0) > 0 as is_personal,
                 v.vehicle_number, r.is_paper_verified, COALESCE(r.is_split_receipt, FALSE)
            FROM receipts r
