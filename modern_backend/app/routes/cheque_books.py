...
Allows tracking and updating cheques by bank, number, status (NSF, void,
cleared, etc.)
"""
        ...
        for row in results:
            summaries.append(
                ChequeBookSummary(
                    ...
                    cheque_range=(
                        f"#{row['min_cheque']}-#{row['max_cheque']}"
                        if row["min_cheque"] and row["max_cheque"]
                        else "N/A"
                    ),
...
async def search_cheques(search: ChequeSearchRequest):
    """Search for cheques by number, amount, payee, bank, status, or date"
    "range"""
...
async def update_cheque(transaction_id: int, update: ChequeUpdateRequest):
    ...
    try:
        ...
        if not result:
            raise HTTPException(
                status_code=404,
                detail=f"Cheque transaction {transaction_id} not found",
            )
