...
class ChartRequest(BaseModel):
    ...
    vehicle_type_requested: Optional[str] = Field(None, description="Vehicle type preference")
    ...
    # Itinerary
    itinerary: List[RouteItem] = Field(..., min_items=2, description="Min 2 stops (pickup + dropoff)")
    
    # Notes
    customer_notes: Optional[str] = Field(None, description="Notes for customer")
    dispatcher_notes: Optional[str] = Field(None, description="Internal notes for dispatcher")
    special_requests: Optional[str] = Field(None, description="Special requests (alcohol, AV, etc)")
    
    # Pricing
    base_charge: Decimal = Field(..., gt=0, description="Base charge amount")
    airport_fee: Decimal = Field(default=Decimal("0.00"), ge=0, description="Airport fee (if applicable)")
    additional_charges_amount: Decimal = Field(default=Decimal("0.00"), ge=0, description="Additional charges")
    ...
    # Status
    status: str = Field(default="Quote", description="Quote, Confirmed, Assigned, In Progress, Completed, Cancelled")
    cancellation_reason: Optional[str] = Field(None, description="Reason for cancellation")
    reference_number: Optional[str] = Field(None, description="Customer reference (PO, etc)")
    ...
    def validate_status(cls, v):
        ...
        valid_statuses = ['Quote', 'Confirmed', 'Assigned', 'In Progress', 'Completed', 'Cancelled']
...
GST_DIVISOR = Decimal("1.05")  # Tax-included formula: gst = total * 0.05 / 1.05

VALID_STATUSES = ['Quote', 'Confirmed', 'Assigned', 'In Progress', 'Completed', 'Cancelled']
VALID_ROUTE_TYPES = ['pickup', 'dropoff', 'stop', 'depart', 'return']

# Payment methods
VALID_PAYMENT_METHODS = ['cash', 'check', 'credit_card', 'debit_card', 'bank_transfer', 'trade_of_services', 'unknown']
