...
def _query_legacy_section(
    conn, table: str, columns: list[str], where_clause: str = "", params: tuple[Any, ...] = ()
) -> tuple[list[str], list[tuple[Any, ...]]]:
...
def _build_sections(
...
) -> dict[str, tuple[list[str], list[tuple[Any, ...]], list[str]]]:
    ...
    if _table_exists(conn, "general_ledger"):
        ...
        if (
            _has_column(conn, "general_ledger", "gifi_code")
...
def generate_notes_to_auditor(
    ...
    lines.append("")
    lines.append("System: limousine dispatch, payroll, invoicing, and reporting platform")
    ...
    unknowns = [
        f for f in getattr(report, "findings", []) if getattr(f, "requires_confirmation", False)
    ]
    if unknowns:
    ...
    lines.append(
        "- Generated package and source records are intended to be retained for 6+ years (general rule; confirm with your retention policy)."
    )
...
def generate_year_end_package(
    ...
    for section_name, (headers, rows, sources) in sections.items():
        ...
        _write_pdf(
            pdf_path,
            f"{section_name} ({request.fiscal_year})",
            [
                f"Rows: {len(rows)}",
        ...
        if not xlsx_ok:
            xlsx_path.write_text(
                "XLSX generation requires openpyxl to be installed.", encoding="utf-8"
            )
