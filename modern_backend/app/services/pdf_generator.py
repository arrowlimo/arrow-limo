...
class CharterPDFForm:
    ...
    def generate(self):
        ...
        doc = SimpleDocTemplate(
            ...
            title=f"Charter Invoice - {self.data.get('reserve_number', 'TBD')}"
        )
        ...
        header_data = [
            [
                Paragraph("<b>ALMS Charter Services</b><br/>Charter Invoice & Receipt", title_style),
                Paragraph(
                    f"<b>Invoice #:</b> {self.data.get('reserve_number', 'TBD')}<br/>"
                    f"<b>Date:</b> {self._format_date(self.data.get('charter_date'))}",
                    normal_style
                )
            ]
        ]
        ...
        res_data = [
            [
                ...
                Paragraph(self._format_date(self.data.get('charter_date')), normal_style)
            ],
            [
                Paragraph("<b>Status:</b>", normal_style),
                Paragraph(self.data.get('status', 'Active'), normal_style),
                Paragraph("<b>Type:</b>", normal_style),
                Paragraph(self._format_charter_type(self.data.get('charter_type')), normal_style)
            ],
            [
                Paragraph("<b>Passengers:</b>", normal_style),
                Paragraph(str(self.data.get('passenger_load', 'TBD')), normal_style),
                Paragraph("<b>Reconciliation:</b>", normal_style),
                Paragraph(self.data.get('reconciliation_status', 'Not Reconciled'), normal_style)
            ]
        ]
        res_table = Table(res_data, colWidths=[1.5*inch, 1.5*inch, 1.5*inch, 1.5*inch])
        ...
        veh_data = [
            [
                Paragraph(f"<b>Vehicle:</b><br/>{self.data.get('vehicle', 'Not assigned')}", normal_style),
                Paragraph(f"<b>Driver:</b><br/>{self.data.get('driver_name', 'Not assigned')}", normal_style),
                Paragraph(f"<b>Capacity:</b><br/>{self.data.get('vehicle_capacity', 'N/A')} pax", normal_style)
            ]
        ]
        ...
        charges_data = [
            ...
            ['Charter Service', '1', f"${float(self.data.get('total_amount_due', 0)):.2f}", 
             f"${float(self.data.get('total_amount_due', 0)):.2f}"]
        ]
        
        if float(self.data.get('nrr_amount', 0)) > 0:
            charges_data.append([
                'NRR/Retainer (Non-Refundable)', '1',
                f"${float(self.data.get('nrr_amount', 0)):.2f}",
                f"${float(self.data.get('nrr_amount', 0)):.2f}"
            ])
        
        charges_table = Table(charges_data, colWidths=[2.5*inch, 0.75*inch, 1.5*inch, 1.25*inch])
        ...
        payment_table.setStyle(TableStyle([
            ...
            ('LINEABOVE', (0, -1), (-1, -1), 1, colors.HexColor('#007bff')),
            ('TOPPADDING', (0, 0), (-1, -1), 4),
            ('BOTTOMPADDING', (0, 0), (-1, -1), 4),
        ]))
        ...
        # GL CODES (if present)
        if self.data.get('gl_revenue_code') or self.data.get('gl_expense_code'):
            story.append(Paragraph("<b>BILLING & GL CODES</b>", section_style))
            gl_data = [
        ...
        if self.data.get('charter_type') == 'exchange_of_services':
            story.append(Paragraph("<b>EXCHANGE OF SERVICES</b>", section_style))
            exch_details = self.data.get('exchange_of_services_details', {})
            exch_text = f"""
            <b>Service Provided:</b> {exch_details.get('service_provided', 'Not specified')}<br/>
            <b>Service Provider:</b> {exch_details.get('service_provider', 'Not specified')}<br/>
            <b>Exchange Value:</b> ${float(exch_details.get('exchange_value', 0)):.2f}<br/>
            <b>Description:</b> {exch_details.get('description', 'No description')}
            """
            story.append(Paragraph(exch_text, normal_style))
            story.append(Spacer(1, 0.2*inch))
        
        # FOOTER
        footer_text = "Thank you for your business! | This is an automated invoice. Please contact our office for questions."
