...
class T2DataExtractor:
    ...
    def extract_expense_data(self, tax_year: int) -> dict:
        ...
        try:
            ...
            cur.execute(
                ...
                FROM receipts r
                LEFT JOIN chart_of_accounts coa ON r.gl_account_code =
                coa.account_code
                WHERE EXTRACT(YEAR FROM r.receipt_date) = %s
                AND r.category NOT LIKE '%%Income%%'
                AND r.category != 'revenue'
                GROUP BY r.gl_account_code, coa.account_name, coa.account_type,
                r.category
            ...
            return {
                ...
                "by_account_type": [
                    {"account_type": row[0], "count": row[1], "amount": row[2]}
    ...
    def extract_t2_deductibility_analysis(self, tax_year: int) -> dict:
        """Analyze T2 deductibility and Schedule 1 add-backs from"
        "receipt-level GL data."""
        ...
        try:
            ...
            for row in rows:
                ...
                if (
                    exclude_from_reports
                    or is_personal_purchase
                    or Decimal(str(owner_personal_amount or 0)) > 0
                    or is_personal_class
                ):
...
def main():
    ...
    try:
        ...
        print(
            f"  Total Assets:          "
            f"${package['balance_sheet']['assets']['total_assets']:>15,.2f}"
        )
        print(
            f"  Accounts Payable:      "
        ...
        if package["tax_rates"]:
            ...
            print(
                ...
                f"{package['tax_rates']['combined_general']*100:>15.1f}%"
            )
            print(
