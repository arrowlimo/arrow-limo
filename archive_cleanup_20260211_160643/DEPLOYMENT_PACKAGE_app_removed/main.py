...
from PyQt6.QtCore import QDate, QDateTime, QSettings, Qt, QTime, QTimer, pyqtSignal
...
class VendorSelector(QComboBox):
    ...
    def _update_validation_color(self):
        ...
        is_in_list = any(vendor_upper == self.itemText(i).upper()
    ...
    def _set_field_style(self, state):
        ...
        if state == 'valid':
            ...
            line_edit.setStyleSheet(
                "QLineEdit { border: 2px solid #4CAF50; background-color: #f0fdf4;}")
        elif state == 'warning':
            # Yellow border and subtle yellow background
            line_edit.setStyleSheet(
                "QLineEdit { border: 2px solid #FFC107; background-color: #fffbf0;}")
        elif state == 'error':
            # Red border and subtle red background
            line_edit.setStyleSheet(
                "QLineEdit { border: 2px solid #f44336; background-color: #fdf0f0;}")
        else:  # neutral
...
class CurrencyInput(QLineEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._validation_state = 'valid'
        self.setStyleSheet(
            "QLineEdit { border: 1px solid #ccc; background-color: white; text-align: right;}")
    ...
    def _set_field_style(self, state):
        ...
        if state == 'valid':
            self.setStyleSheet(
                "QLineEdit { border: 2px solid #4CAF50; background-color: #f0fdf4; text-align: right;}")
        elif state == 'warning':
            self.setStyleSheet(
                "QLineEdit { border: 2px solid #FFC107; background-color: #fffbf0; text-align: right;}")
        elif state == 'error':
            self.setStyleSheet(
                "QLineEdit { border: 2px solid #f44336; background-color: #fdf0f0; text-align: right;}")
        else:  # neutral
            self.setStyleSheet(
                "QLineEdit { border: 1px solid #ccc; background-color: white; text-align: right;}")
    ...
    def _do_format(self):
        ...
        if "." in cleaned:
            ...
            cents = parts[1][:2] if len(
                parts) > 1 else "00"  # Limit to 2 decimals
...
class DatabaseConnection:
    ...
    def __init__(self):
        try:
            ...
            if password:
                ...
                print("[DB] Using Windows integrated authentication (SSPI)", flush=True)
            else:
    ...
    def get_cursor(self):
        ...
        try:
            ...
            if status == extensions.TRANSACTION_STATUS_INERROR:
                # Silently cleanup error state - this is normal after query errors
                self.conn.rollback()
    ...
    def _reconnect(self):
        ...
        try:
            ...
            print(f"[DB ERROR] Reconnection failed: {e}", flush=True)
            raise
...
class CharterFormWidget(QWidget):
    ...
    QTableWidget.SelectionMode.SingleSelection)
        ...
        self.payments_table.setHorizontalHeaderLabels(
            ["Payment Type", "Amount", "Method", "Notes"])
    ...
    f"Day {
        idx+
        1}: on-duty {hours}h > {daily_on_limit}h")
    ...
    f"Cycle 2: 14-day total {last14}h > 120h")
            ...
            if not violations:
                ...
                reset_note = f"; reset at day {
    reset_index+1}" if reset_index != -1 else ""
    ...
    self, "No Form", "No inspection form has been uploaded yet.")
            ...
            if not os.path.exists(self.current_inspection_form_path):
                QMessageBox.warning(
    self, "Not Found", f"Inspection form file not found:\n{
    ...
    self,
     'exemption_off_duty_deferral') and self.exemption_off_duty_deferral.isChecked():
            ...
            html = [
                ...
                f"{
    cb(no_defects)} No Defects &nbsp; {
        cb(minor_def)} Minor Defects &nbsp; {
            cb(major_def)} Major Defects",
                "</div>",
                f"<div class='row'><span class='lbl'>Defect Notes:</span><br>{
    defect_notes.replace(
        '<', '&lt;').replace(
            '>', '&gt;').replace(
                '\n', '<br>')}</div>",
                f"<div class='row'><span class='lbl'>Exemptions:</span> {
    '; '.join(exemptions) if exemptions else 'None'}</div>",
    ...
    ch for ch in driver if ch.isalnum() or ch in (
        '-', '_')).strip('_') or 'driver'
            safe_vehicle = ''.join(
    ch for ch in vehicle if ch.isalnum() or ch in (
        '-', '_')).strip('_') or 'vehicle'
    ...
    self, "Run Types", "Table charter_run_types not found.")
            ...
            cur.execute(f"""
                ...
                FROM charter_run_types
                ORDER BY {('display_order, ' if has_order else '')}run_type_name
            """)
            rows = cur.fetchall()
            cur.close()
    ...
    def _build_hos_log_html(self):
        ...
        header_dates = ''.join(
    f"<th>{
        self.hos_last14_dates[c].strftime('%Y-%m-%d')}</th>" for c in range(14))
        html = [
            "<html><head><meta charset='utf-8'><style>"
    ...
    to_addr.strip(), subject, body, pdf_path)
            QMessageBox.information(
    self, 'Email Sent', f'HOS PDF emailed to {
        to_addr.strip()}')
        except Exception as e:
            QMessageBox.warning(
    self, 'Email Error', f'Failed to send email: {e}')
    ...
    to_addr.strip(), subject, body, pdf_path)
            QMessageBox.information(
    self, 'Text Sent', f'HOS PDF sent to {
        to_addr.strip()} (via MMS gateway)')
        except Exception as e:
            QMessageBox.warning(
    self, 'Text Error', f'Failed to send text: {e}')
     ...
     "Drop-off cannot be before pickup. Adjust the date/time (multi-day allowed).")
        ...
        try:
            ...
            if self.charter_id:
                ...
                QMessageBox.information(
    self, "Success", f"Charter #{
        self.charter_id} updated successfully")
                ...
                cur.execute(
                    ...
                    INSERT INTO charters (
                        reserve_number, charter_date, pickup_time, passenger_count, notes, status, client_id, is_out_of_town, charter_data) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s::jsonb)
    ...
    self.current_inspection_form_path, os.path.dirname(__file__))
                ...
                if hasattr(self, "reserve_number"):
                    try:
                        ...
                        pass
                QMessageBox.information(
    self, "Success", f"New charter created!\n\nReserve #: {reserve_num}\nCharter ID: {
    ...
    reserve_num, start_dt, end_dt, customer_data.get(
        "client_name", ""))
            ...
            self.db.rollback()
            QMessageBox.critical(
    self, "Database Error", f"Failed to save charter:\n\n{
        e.diag.message_primary if hasattr(
            e, 'diag') else str(e)}",)
        except Exception as e:
            self.db.rollback()
            QMessageBox.critical(
    ...
    escrow_info: dict,
     cur):
        ...
        try:
            ...
            print(
    f"✅ GL coded escrow NRR: ${
        nrr_amount:.2f} from cancelled reserve #{from_reserve}")
    ...
    end_dt,
     customer_name: str):
        ...
        try:
            ...
            appt.Subject = f"Reserve {reserve_number} - {
    customer_name or 'Charter'}"
            appt.Start = start_dt
            appt.End = end_dt
    ...
    def print_confirmation(self):
        ...
        try:
            ...
            text = f"{
    datetime.now().strftime('%m/%d/%Y')}\t\t\t\t\tYour Reservation Number is {reserve_num}.\n"
            ...
            # Service summary
            text += f"Date of Service: {
    self.charter_date.getDate().toString('MM/dd/yyyy')}\n"
    ...
    "hourly", {}).get(
        "minimum_hours", 0.0)
            ...
            # Build quote letter
            text = f"{
    datetime.now().strftime('%m/%d/%Y')}\t\t\t\t\tYour Quote Number is {reserve_num}.\n"
        ...
        "extra_time_rate", hourly_rate))
            ...
            if options["mode"] == "custom":
                ...
                if options.get("hourly"):
                    ...
                    text += f"This option charges ${
    hourly_rate:.2f} for each hour of service.\n"
                ...
                if options.get("package"):
                    ...
                    if extra_hours > 0:
                        text += f"Extra Time: {extra_hours} hours @ ${
    extra_time_rate:.2f}/hour = ${
        extra_cost:.2f}\n"
                    ...
                    text += f"Additional time beyond {package_hours} hours: ${
    extra_time_rate:.2f}/hour.\n\n"
                ...
                if options.get("split"):
                    ...
                    if standby_hours > 0:
                        text += f"Driver Standby/Waiting: {standby_hours} hours @ ${
    standby_rate:.2f}/hour = ${
        split_total:.2f}\n"
                    else:
                    ...
                    text += f"Driver waits during event. Standby time charged at ${
    standby_rate:.2f}/hour.\n\n"
    ...
    def generate_client_beverage_html(self) -> str:
        ...
        if deposit > 0:
            html += f"<tr><td colspan='3'><b>Deposit/Recycle Fees</b></td><td>-</td><td>${
    deposit:.2f}</td></tr>"
            total += deposit

        html += f"<tr><td colspan='3'><b>Subtotal</b></td><td><b>${
    total_gst:.2f}</b></td><td><b>${
        total:.2f}</b></td></tr>"
    ...
    "DELETE FROM charter_routes WHERE charter_id = %s", (self.charter_id,))
            ...
            print(
    f"✅ Saved {
        self.route_table.rowCount()} routes for charter {
            self.charter_id}")
        except Exception as e:
            try:
                self.db.rollback()
    ...
    total_item.text().replace(
        '$', '').replace(
            ...
            print(
    f"✅ Saved {
        self.charges_table.rowCount()} charges for charter {
            self.charter_id}")
        except Exception as e:
            try:
                self.db.rollback()
        ...
        try:
            ...
            if not events:
                ...
                try:
                    cur.execute(
                        """
                        SELECT pickup_address, dropoff_address, pickup_time, dropoff_time
                        FROM charters
                        WHERE charter_id = %s
                        """,
                        (charter_id,))
                    ...
                    if row:
                        ...
                        _set_parent_row(0, pickup_addr, pickup_time, "")
                        _set_parent_row(1, dropoff_addr, dropoff_time, "")
                        print(
    ...
    f"{
        item['item_name']:<40} {
            item['quantity']:<5} ${
                item['unit_price_charged']:<11.2f} ${
                    item['line_amount_charged']:<11.2f}")
...
class AccountingReceiptsWidget(QWidget):
    ...
    def _show_error(self, error_msg: str):
        ...
        error_display = QLabel(f"❌ Error Loading Receipts Module\n\n{error_msg}")
        ...
        error_display.setAlignment(Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft)
    ...
    def init_ui(self):
        ...
        print(
    "    >> [init_ui] Creating ReceiptSearchMatchWidget...",
     flush=True)
        ...
        try:
            ...
            import traceback
            print(
    f"    >> ❌ ReceiptSearchMatchWidget failed to init: {e}",
     flush=True)
            traceback.print_exc()
            raise
        print(
    "    >> [init_ui] ReceiptSearchMatchWidget created, setting parent_tab_widget...",
     flush=True)
        ...
        print(
    "    >> [init_ui] ReceiptSearchMatchWidget tab added OK",
     flush=True)
    ...
    def _create_add_receipt_tab(self):
        ...
        self.tax_jurisdiction.setToolTip(
            "<b>Tax Jurisdiction</b><br>Select province/state for automatic tax calculation.<br>Default: AB (GST 5%)")
        ...
        self.pst_input.setToolTip(
            "<b>PST / Additional Sales Tax</b><br>Enter PST (BC, SK, MB, QC) or US state sales tax.<br>Auto-calculated for Canadian provinces.")
        ...
        self.gl_combo.setToolTip(
            "<b>GL Account Code</b><br>General Ledger account for accounting.<br>Auto-filled from vendor history if available.")
        self.gl_combo.currentIndexChanged.connect(self._maybe_set_gst_exempt_from_gl)

        self.vehicle_combo = QComboBox()
        self.vehicle_combo.addItem("", None)
        self.vehicle_combo.setToolTip(
            "<b>Vehicle</b><br>Optional: Link expense to specific vehicle<br>(e.g., for fuel or maintenance)")
        ...
        self.fuel_amount_input.setToolTip(
            "Fuel amount in liters (optional).")
        ...
        self.gst_exempt_check.setToolTip(
            "Check for GST-exempt items (e.g., WCB, government services, basic groceries)")
    ...
    def _maybe_set_gst_exempt_from_gl(self):
    ...
    def auto_calc_gst(self):
        ...
        try:
            ...
            if "AB" in jurisdiction or "YT" in jurisdiction or "NT" in jurisdiction or "NU" in jurisdiction:
                ...
                pst_str = self.pst_input.get_value() if hasattr(
                    self.pst_input, 'get_value') else "0.00"
    ...
    def load_vehicles(self):
        try:
            ...
            status_column = "status" if "status" in vehicle_columns else "operational_status"
            if status_column not in vehicle_columns:
                status_column = "status"

            cur.execute(
                rf"""
                SELECT vehicle_id, vehicle_number, {status_column} AS status, vehicle_type
                FROM vehicles
                ORDER BY
                    CASE WHEN LOWER(COALESCE({status_column}, '')) = 'active' THEN 0 ELSE 1 END,
                    CASE
                        WHEN vehicle_number ~ '^[Ll]-?\d+$' THEN CAST(regexp_replace(vehicle_number, '[^0-9]', '', 'g') AS INT)
                        ELSE 9999
                    END,
    ...
    def _update_receipt_vehicle_type_display(self):
        try:
            ...
            if hasattr(
    self,
     "_receipt_vehicle_types") and vid in self._receipt_vehicle_types:
    ...
    def load_receipts(self, filters=None):
        """Load receipts with optional filters, multiple date ranges (OR), and optional year grouping"""
        if filters is None:
    ...
    "FROM receipts ")
            ...
            should_group_by_year = filters.get(
    "group_by_year", False) if filters else False

            base_query += "ORDER BY receipt_date DESC, receipt_id DESC LIMIT 500"
            ...
            if should_group_by_year:
                ...
                for year in sorted(rows_by_year.keys(), reverse=True):
                    # Add year header row
                    display_rows.append(
    (None, None, f"═══ {year} ═══", None, None, None, None, None))
    ...
    def apply_receipt_filters(self):
        ...
        amt_min_text = self.filter_amount_min.text().replace(
            "$", "").replace(",", "").strip()
        if amt_min_text:
            try:
                ...
                return
        amt_max_text = self.filter_amount_max.text().replace(
            "$", "").replace(",", "").strip()
        if amt_max_text:
            try:
    ...
    def _get_default_receipt_filters(self):
    ...
    def _on_receipt_filters_changed(self, filters_list):
    ...
    amount_max_text.replace(
        ...
        if date_ranges:
            filters["date_ranges"] = [
    (from_date.toPyDate(),
    to_date.toPyDate()) for from_date,
     to_date in date_ranges]

        # Group by year option
        filters["group_by_year"] = self.date_filter_builder.should_group_by_year()
    ...
    def _on_receipt_cell_changed(self, row, column):
        ...
        value_text = self.table.item(
    row, column).text() if self.table.item(
        row, column) else ""
        try:
        ...
        try:
            ...
            cur = self.db.get_cursor()
            cur.execute(
    f"UPDATE receipts SET {field} = %s WHERE receipt_id = %s",
    (parsed_value,
     receipt_id))
            self.db.commit()
        except Exception as e:
    ...
    def _get_selected_receipt_rows(self):
        ...
        for row in range(self.table.rowCount()):
            ...
            if isinstance(
    checkbox,
     QTableWidgetItem) and checkbox.checkState() == Qt.CheckState.Checked:
                selected.append(row)
        return selected
    ...
    self, "Batch Category", "Select new category:", categories, 0, False)
    ...
    self, "Bulk Verify", "No receipts selected")
        ...
        try:
            ...
            for row in rows:
                ...
                if receipt_id:
                    cur.execute(
    "UPDATE receipts SET reviewed = TRUE WHERE receipt_id = %s", (receipt_id,))
                    # Visual feedback
            ...
            self.db.commit()
            QMessageBox.information(
    self, "Success", f"Marked {
        len(rows)} receipts as verified")
        except Exception as e:
            self.db.rollback()
    ...
    def save_receipt(self):
        ...
        try:
            ...
            progress.close()
            QMessageBox.warning(
    self, "Validation", "Amount must be a valid number")
            return

        gst_val = Decimal(
            str(self.gst_display.text().replace('$', '').replace(',', '') or '0'))
        pst_val = Decimal(str(self.pst_input.get_value())) if hasattr(
            self.pst_input, 'get_value') else Decimal('0')
        gl_code = self.gl_combo.currentData()
        gl_name = self.gl_accounts.get(gl_code, None) if gl_code else None
        vehicle_id = self.vehicle_combo.currentData()
        vehicle_id = int(vehicle_id) if vehicle_id else None
        fuel_amount = self.fuel_amount_input.value() if hasattr(
            self, "fuel_amount_input") else 0
        ...
        tax_jurisdiction = self.tax_jurisdiction.currentText() if hasattr(
            self, "tax_jurisdiction") else None
        category = None
        ...
        owner_personal_amount = amount if is_personal and not is_driver_personal else Decimal(
            "0")
        ...
        try:
            ...
            cur.execute(
                ...
                INSERT INTO receipts (
                    ...
                    gl_account_name) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                RETURNING receipt_id
                """,
                (
    ...
    self, "Saved", f"Receipt #{receipt_id} saved")
            ...
            progress.close()
            QMessageBox.critical(
    self, "Save Failed", f"Could not save receipt:\n{e}")
...
class CustomersWidget(QWidget):
    ...
    def load_selected_customer(self):
        ...
        try:
            ...
            QMessageBox.warning(
    self, "Load Error", f"Failed to load customer: {e}")
    ...
    def load_customers(self):
        ...
        try:
            ...
            try:
                ...
                pass
            QMessageBox.warning(
    self, "Load Error", f"Failed to load customers: {e}")
    ...
    def save_customer(self):
        ...
        if not name:
            QMessageBox.warning(
    self, "Validation", "Customer name is required")
            self.name_input.setFocus()
            return
        ...
        try:
            ...
            if self.current_customer_id:
                ...
                self.db.commit()
                QMessageBox.information(
    self, "Saved", f"Customer #{
        self.current_customer_id} updated")
            else:
    ...
    self,
    "Saved",
     f"Customer #{customer_id} (Account #{new_account_number}) created")
            self.load_customers()
            self.search_input.clear()
        except Exception as e:
            self.db.rollback()
            QMessageBox.critical(
    self, "Save Failed", f"Could not save customer:\n{e}")
    ...
    f"Delete customer #{
        ...
        if response == QMessageBox.StandardButton.Yes:
            try:
                ...
                cur = self.db.get_cursor()
                cur.execute(
    "DELETE FROM clients WHERE client_id = %s", (self.current_customer_id,))
                self.db.commit()
                ...
                self.db.rollback()
                QMessageBox.critical(
    self, "Delete Failed", f"Could not delete customer:\n{e}")
...
class MainWindow(QMainWindow):
    ...
    db: Optional[DatabaseConnection] = None,
     auth_user: Optional[Dict] = None):
        ...
        user_suffix = f" - {
    self.auth_user.get('username')}" if self.auth_user.get("username") else ""
        self.setWindowTitle(
    ...
    "Database Error",
     f"Cannot connect to database:\n{e}")
        ...
        try:
            ...
            print(
    f"  [WARN] Warning: Error logger initialization failed: {e}",
     flush=True)
    ...
    f"User: {user_display} | Role: {role_display}")
        ...
        print(
    "  9. Creating Operations tab (placeholder for lazy loading)...",
     flush=True)
        try:
            ...
            print("  10. Operations tab placeholder OK", flush=True)
        except Exception as e:
        ...
        try:
            self.tabs.addTab(
    self.create_fleet_people_parent_tab(),
     "🚗 Fleet Management")
        ...
        print(
    "  13. Creating Accounting tab (placeholder for lazy loading)...",
     flush=True)
        try:
            # Create placeholder - real widgets created on first click
            ...
            print("  14. Accounting tab placeholder OK", flush=True)
        except Exception as e:
        ...
        try:
            ...
            allowed_admin_roles = {
    "admin", "management", "manager", "super_user"}
            if self.auth_user and str(
    self.auth_user.get(
        "role",
         "")).lower() not in allowed_admin_roles:
        ...
        if hasattr(
    self,
    'enhanced_charter_widget') and hasattr(
        self,
         'operations_tabs'):
    ...
    def _on_tab_changed(self, index: int):
        ...
        if tab_text in self._tabs_loaded or tab_text in self._tab_load_in_progress:
            return

        self._tab_load_in_progress.add(tab_text)
        print(f"[LOAD] Loading tab: {tab_text}", flush=True)
        ...
        try:
            ...
            print(f"[LOAD] {tab_text} loaded successfully", flush=True)
        except Exception as e:
            print(f"[ERROR] Failed to load {tab_text}: {e}", flush=True)
    ...
    def undo_action(self):
        ...
        QMessageBox.information(
    self, "Undo", "Undo functionality coming soon\n[Undo stack pending]")
    ...
    def closeEvent(self, event):
        """Handle application shutdown gracefully - prevents crash on VS Code close"""
        try:
            # Stop session timer
            ...
            print(f"Warning during shutdown: {e}", flush=True)
    ...
    self,
    "Launch Error",
     f"Error launching {display_name}:\n{e}")
    ...
    def create_accounting_parent_tab(self) -> QWidget:
        ...
        self.safe_add_tab(
    tabs, VendorInvoiceManager(
        self.db), "📋 Vendor Invoice Manager")
        self.safe_add_tab(
    tabs,
    self.create_payroll_entry_tab(),
     "🧾 Payroll Entry")
        self.safe_add_tab(
    tabs,
    self.create_tax_management_tab(),
     "🏛️ Tax Management")
        self.safe_add_tab(tabs, WCBRateEntryWidget(self.db), "🛡️ WCB Rates")
        self.safe_add_tab(
    tabs,
    self.create_business_entity_tab(),
     "🏢 Business Entity")
    ...
    def create_admin_parent_tab(self) -> QWidget:
        ...
        self.safe_add_tab(
    tabs, BeverageManagementWidget(
        self.db), "🍷 Beverage Management")
            ...
            if event_data.get('driver'):
                if hasattr(charter_form, 'driver_combo'):
                    for i in range(charter_form.driver_combo.count()):
                        if event_data['driver'] in charter_form.driver_combo.itemText(
                            i):
            ...
            if event_data.get('vehicle'):
                if hasattr(charter_form, 'vehicle_combo'):
                    for i in range(charter_form.vehicle_combo.count()):
                        if event_data['vehicle'] in charter_form.vehicle_combo.itemText(
                            i):
    ...
    def create_dispatch_tab(self) -> QWidget:
        ...
        dispatch_tabs.addTab(
    self.outlook_calendar_widget,
     "📅 Calendar (Outlook Style)")
        ...
        dispatch_tabs.addTab(
    self.dispatcher_calendar_widget,
     "🗓️ Calendar (Table View)")
        ...
        # TAB 6: Vehicle Booked Out
        self.vehicle_booked_out_widget = VehicleBookedOutWidget(self.db)
        dispatch_tabs.addTab(self.vehicle_booked_out_widget, "🚐 Vehicle Booked Out")

        # TAB 7: Unbooked Calendar Events (click to book)
    ...
    self.customer_worth_widget,
     "⭐ Customer Worth (RFM)")
        ...
        report_tabs.addTab(
    self.realtime_tracking_widget,
     "🗺️ Fleet Tracking Map")
    ...
    self.multi_payroll_widget,
     "💳 Multi-Location Payroll")
        ...
        report_tabs.addTab(
    self.ops_consol_widget,
     "⚙️ Operations Consolidation")
    ...
    def open_find(self):
        ...
        if ok and text:
            QMessageBox.information(
    self, "Search", f'Searching for "{text}"\n[Full implementation pending]')
    ...
    def _show_global_results(self, query: str, receipts, charters, clients):
        ...
        summary = QLabel(
    f"Receipts: {
        len(receipts)} | Charters: {
            len(charters)} | Clients: {
                len(clients)}")
...
def set_active_db(target):
    ...
    try:
        ...
        with log_path.open("a", encoding="utf-8") as fh:
            fh.write(msg + "\n")
        print(f"[DB TARGET] {msg}", flush=True)
    except Exception:
        pass


# Global DB target configuration (defaults to Local database for local app)
...
def main():
    ...
    [
        "powershell",
        "-Command",
        "Get-Service postgresql-x64-17 | Select-Object -ExpandProperty Status"],
        capture_output=True,
        encoding='utf-8',
        errors='ignore',
         timeout=5)
                if "Stopped" in result.stdout:
                    print(
    "[INFO] PostgreSQL is stopped, attempting to start...",
     flush=True)
    ...
    [
        ...
        stdout=subprocess.DEVNULL,
         stderr=subprocess.DEVNULL)
                ...
                print(
    f"[WARNING] Could not auto-start PostgreSQL: {e}",
     flush=True)
        ...
        if auto_login:
            print(f"AUTO_LOGIN enabled - logging in as {auto_login_user}", flush=True)
        ...
        active_db_target=ACTIVE_DB_TARGET,
         set_db_callback=set_active_db)
            ...
            if login_dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                # Refresh DB config after login in case credentials were entered
        ...
        try:
            ...
            if db_host == 'localhost' or db_host == '127.0.0.1':
                mode_label = f"Local DB ({db_name})"
            else:
                mode_label = f"Cloud DB ({db_host}/{db_name})"
            if auto_login:
